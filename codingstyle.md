...
my $count = 0;
my $user_name = "admin";
my $db_handle = get_database();
my $max_retries = 5;
```

Alternative: camelCase is also used in the codebase:
```perl
...
**Best Practice:** Be consistent within a module. Prefer underscores for new code.
...
**Alternative:** Some methods use camelCase (especially for legacy code):
```perl
sub getSettings { ... }
sub checkDBH { ... }
sub updateConfig { ... }
...
```perl
sub function_name($self, $param1, $param2) {
    # function body
}
```
...
For methods:
```perl
sub method_name($self, $arg1, $arg2, %options) {
    # method body
}
```
