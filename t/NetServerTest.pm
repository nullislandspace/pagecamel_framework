...
sub prepare_test {
    ...
    if (!$env{'hostname'}) {
        eval { require PageCamel::Net::Server::Proto } || do { SKIP: { skip("Could not load PageCamel::Net::Server::Proto to lookup host: $@", $N - 1) }; exit; };
        foreach my $host (qw(localhost localhost.localdomain localhost6 * ::1)) { # try local bindings first to avoid opening external ports during testing
            my @info = eval { PageCamel::Net::Server::Proto->get_addr_info($host) };
            next if ! @info;
