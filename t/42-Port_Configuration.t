...
use_ok('PageCamel::Net::Server');

@FooServer::ISA = qw(PageCamel::Net::Server);
...
if (!eval {
  ...
  SKIP: {
    ...
    p_c([], {
        bind => [{
            host => '*',
            port => PageCamel::Net::Server::default_port(),
            ipv  => '4',
            proto => 'tcp',
        }],
        sock => [{
            NS_host => '*',
            NS_port => PageCamel::Net::Server::default_port(),
            NS_ipv  => '4',
            NS_proto => 'TCP',
    ...
    p_c([port => ["localhost,20202,PageCamel::Net::Server::Proto::TCP"]], {
        bind => [{host => 'localhost', port => 20202, proto => 'PageCamel::Net::Server::Proto::TCP', ipv => 4}],
    });
...
if (!eval { require Net::SSLeay; 1 }) {
  ...
  SKIP: {
    ...
    p_c([proto => 'ssleay'], {
        bind => [{host => '*', port => PageCamel::Net::Server::default_port(), proto => 'ssleay', ipv => 4}],
...
if (!eval { require IO::Socket::SSL }) {
  ...
  SKIP: {
    ...
    p_c([proto => 'ssl'], {
        bind => [{host => '*', port => PageCamel::Net::Server::default_port(), proto => 'ssl', ipv => 4}],
