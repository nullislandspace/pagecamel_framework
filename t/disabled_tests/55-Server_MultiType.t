...
package PageCamel::Net::Server::Test;
use strict;
use FindBin qw($Bin);
use lib $Bin;
...
use_ok('PageCamel::Net::Server::MultiType');
@PageCamel::Net::Server::Test::ISA = qw(PageCamel::Net::Server::MultiType);


sub accept {
    $env->{'signal_ready_to_test'}->();
    diag("PageCamel::Net::Server::MultiType::ISA: (".join(",",@PageCamel::Net::Server::MultiType::ISA).")");
...
my $ok = eval {
    ...
    if ($pid) {
        ...
        my $line = <$remote>;
        diag($line);
        die "Didn't get the type of line we were expecting: ($line)" if $line !~ /PageCamel::Net::Server/;
        print $remote "exit\n";
        return 1;
