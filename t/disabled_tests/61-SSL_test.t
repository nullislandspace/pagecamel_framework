...
use_ok qw(PageCamel::Net::Server::Proto::SSL) or exit;
require PageCamel::Net::Server;
@PageCamel::Net::Server::Test::ISA = qw(PageCamel::Net::Server);
...
my $ok = eval {
    ...
    ### child does the server
        ...
        eval {
            alarm $env->{'timeout'};
            close STDERR;
            my $s = PageCamel::Net::Server::Test->run(
                host  => $env->{'hostname'},
                port  => $env->{'ports'}->[0],
                proto => 'ssl',
