...
use PageCamel::Protocol::HTTP2::Constants qw(const_name);
...
sub server {
    ...
    tcp_server $host, $port, sub {
            ...
            eval {
                ...
                if ( $h{npn} ) {
                    ...
                    Net::SSLeay::CTX_set_next_protos_advertised_cb( $tls->ctx,
                        [PageCamel::Protocol::HTTP2::ident_tls] );
                }
                if ( $h{alpn} ) {
        ...
        my $server = PageCamel::Protocol::HTTP2::Server->new(%h);
        $cb->($server);
...
sub client {
    ...
    my $client = PageCamel::Protocol::HTTP2::Client->new(%h);
    $cb->($client);
