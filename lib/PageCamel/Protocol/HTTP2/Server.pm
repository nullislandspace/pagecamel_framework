...
PageCamel::Protocol::HTTP2::Server - HTTP/2 server
...
=head1 SYNOPSIS
    ...
    tcp_server 'localhost', 8000, sub {
        ...
        # Create PageCamel::Protocol::HTTP2::Server object
        my $server;
...
PageCamel::Protocol::HTTP2::Server is HTTP/2 server library. It's intended to make
...
for PSGI based on L<PageCamel::Protocol::HTTP2::Server>.
...
Initialize new server object
    ...
    my $server = PageCamel::Protocol::HTTP2::Client->new( %options );
...
sub new {
    ...
    # Store callback for extended CONNECT (RFC 8441)
    if ( exists $opts{on_connect_request} ) {
        $self->{connect_cb} = delete $opts{on_connect_request};
    }
    ...
    if ( exists $opts{on_request} ) {
        ...
        $opts{on_new_peer_stream} = sub {
            ...
            # For extended CONNECT (RFC 8441), the client sends HEADERS without END_STREAM
            # because it needs to keep the stream open for bidirectional tunnel data.
            # So we need to handle it in OPEN state, not wait for HALF_CLOSED.
            $self->{con}->stream_cb(
                $stream_id,
                OPEN,
                sub {
                    ...
                    if ( $h{':method'} eq 'CONNECT' && exists $h{':protocol'} ) {
                        if ( $self->{connect_cb} ) {
                            ...
                            # No handler for extended CONNECT, reject
                            tracer->warning("Extended CONNECT received but no on_connect_request handler");
                            $self->{con}->stream_error($stream_id, REFUSED_STREAM);
                        }
                    }
                    # Regular requests will be handled in HALF_CLOSED callback
                }
            );

            # Regular requests (with END_STREAM) go to HALF_CLOSED
            $self->{con}->stream_cb(
                $stream_id,
                HALF_CLOSED,
                sub {
    ...
    $self->{con} =
      PageCamel::Protocol::HTTP2::Connection->new( SERVER, %opts,
...
sub response_stream {
    ...
    return PageCamel::Protocol::HTTP2::Server::Stream->new(
        con       => $con,
...
Enable extended CONNECT protocol (RFC 8441) for WebSocket over HTTP/2.
Must be called before sending SETTINGS to client.

    $server->enable_connect_protocol();

Or with explicit value:

    $server->enable_connect_protocol(1);  # enable
    $server->enable_connect_protocol(0);  # disable
...
=head3 tunnel_response

Accept an extended CONNECT request and create a bidirectional tunnel (RFC 8441).
Returns a PageCamel::Protocol::HTTP2::Server::Tunnel object for bidirectional communication.
...
sub tunnel_response {
    ...
    # Mark stream as tunnel mode
    $con->stream_tunnel( $stream_id, 1 );

    # Send headers without END_STREAM (tunnel stays open)
    $con->send_headers(
        $stream_id,
        [
            ':status' => $status,
        ...
        0  # Don't set END_STREAM - tunnel remains open
    );

    return PageCamel::Protocol::HTTP2::Server::Tunnel->new(
        con       => $con,
        stream_id => $stream_id,
        on_data   => $h{on_data},
        on_close  => $h{on_close},
