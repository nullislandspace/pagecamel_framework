package PageCamel::Worker::GardenSpaceProgram::DGN2Decoder;
#---AUTOPRAGMASTART---
use 5.030;
use strict;
use warnings;
...
sub crossregister {
    ...
    $self->{clacks}->listen('GSP::RECIEVE::AD'); # Only listen for DGN2 frames
...
sub work {
    ...
    while((my $message = $self->{clacks}->getNext())) {
        ...
        next unless($message->{name} eq 'GSP::RECIEVE::AD');
...
sub decodeStatusFrame {
    ...
    my $insth = $dbh->prepare_cached("INSERT INTO gsp.dgn2_status
    ...
    my $clacksdata = join(',', @parts);
    $self->{clacks}->set('GSP::DGN2::STATUS', $clacksdata);
    $self->{clacks}->doNetwork();
    $reph->debuglog("DGN2 status frame: $clacksdata");
...
sub decodeDHTFrame {
    ...
    my $insth = $dbh->prepare_cached("INSERT INTO gsp.dgn2_dht
    ...
    my $clacksdata = join(',', @parts);
    $self->{clacks}->set('GSP::DGN2::DHT', $clacksdata);
    $self->{clacks}->setAndStore('GSP::DGN2::TEMPERATURE', 0 + $decoded{temperature});
    $self->{clacks}->setAndStore('GSP::DGN2::HUMIDITY', 0 + $decoded{humidity});
    $self->{clacks}->doNetwork();
    $reph->debuglog("DGN2 DHT frame: $clacksdata");
