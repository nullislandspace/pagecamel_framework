package PageCamel::Worker::GardenSpaceProgram::BedroomSensorDecoder;
#---AUTOPRAGMASTART---
use 5.030;
use strict;
use warnings;
...
sub crossregister {
    ...
    $self->{clacks}->listen('GSP::RECIEVE::FC'); # Only listen for BedroomSensor frames
...
sub work {
    ...
    while((my $message = $self->{clacks}->getNext())) {
        if($message->{type} eq 'disconnect') {
            $self->{clacks}->listen('GSP::RECIEVE::FC'); # Only listen for WaterFeeler frames
            $self->{clacks}->ping();
            $self->{clacks}->doNetwork();
            $self->{nextping} = $now + 30;
            next;
        ...
        next unless($message->{name} eq 'GSP::RECIEVE::FC');
...
sub decodeFrame {
    ...
    if($frame[6] != 0x52) {
        # Not originating from BedroomSensor
        return 0;
    }
    ...
    if($frame[8] == 1) {
        ...
        $self->{clacks}->set('GSP::BEDROOMSENSOR::STATECHANGE', $clacksdata);
    } elsif($frame[8] == 36) {
        # powersave shutdown
        my %decoded;
        $decoded{statechange_timestamp} = getISODate();
...
sub decodeStatusFrame {
    ...
    my $insth = $dbh->prepare_cached("INSERT INTO gsp.bedroomsensor_status
    ...
    my $clacksdata = join(',', @parts);
    $self->{clacks}->set('GSP::BEDROOMSENSOR::STATUS', $clacksdata);
    $self->{clacks}->doNetwork();
    $reph->debuglog("BEDROOMSENSOR status frame: $clacksdata");
...
sub decodeDHTFrame {
    ...
    my $clacksdata = join(',', @parts);
    $self->{clacks}->set('GSP::BEDROOMSENSOR::DHT', $clacksdata);
    $self->{clacks}->setAndStore('GSP::BEDROOMSENSOR::TEMPERATURE', 0 + $decoded{temperature});
    $self->{clacks}->setAndStore('GSP::BEDROOMSENSOR::HUMIDITY', 0 + $decoded{humidity});
    $self->{clacks}->doNetwork();
    $reph->debuglog("BEDROOMSENSOR DHT frame: $clacksdata");
