package PageCamel::Worker::GardenSpaceProgram::GEO1BDecoder;
#---AUTOPRAGMASTART---
use 5.020;
use strict;
use warnings;
...
sub crossregister {
    ...
    $self->{clacks}->listen('GSP::RECIEVE::BD'); # Only listen for GEO-1B frames
...
sub work {
    ...
    my $insth = $dbh->prepare_cached("INSERT INTO gsp.geo1b_status
    ...
    while((my $message = $self->{clacks}->getNext())) {
        ...
        next unless($message->{name} eq 'GSP::RECIEVE::BD');
...
sub decodeFrame {
    ...
    $self->{clacks}->set('GSP::GEO1B::STATUS', $clacksdata);
    $self->{clacks}->doNetwork();
    $reph->debuglog("GEO-1B status frame: $clacksdata");
