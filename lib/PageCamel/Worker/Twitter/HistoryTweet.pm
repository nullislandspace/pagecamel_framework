package PageCamel::Worker::Twitter::HistoryTweet;
#---AUTOPRAGMASTART---
use 5.030;
use strict;
use warnings;
...
sub work {
    ...
    #$reph->debuglog($self->{modname} . " checking for new events.\n");

    my $selsth = $dbh->prepare_cached("SELECT * FROM " . $self->{table} . "
                                        WHERE replaytime <= now()
                                        AND is_tweeted = false
                                        ORDER BY replaytime
                                        LIMIT 1")
        or croak($dbh->errstr);
    ...
    $selsth->finish;
    if(!defined($tweet) || !defined($tweet->{logtime})) {
        $dbh->rollback;
        return 0;
    }

    my $insth = $dbh->prepare_cached("INSERT INTO twitter_outbox
                                     (message, account_name, starttime)
                                     VALUES (?, ?, ?)
                                     RETURNING tweet_id")
            or croak($dbh->errstr);

    my $upsth = $dbh->prepare_cached("UPDATE " . $self->{table} . "
                                     SET is_tweeted = true
                                     WHERE logtime = ?")
            or croak($dbh->errstr);

    if(!$insth->execute($tweet->{tweettext}, $self->{twitter_account}, $tweet->{replaytime})) {
    ...
    if(!$upsth->execute($tweet->{logtime})) {
    ...
    $dbh->commit;
    $reph->debuglog($self->{twitter_account} . ' tweeted: ' . $tweet->{tweettext});
    $reph->debuglog("Tweeted with outbox id $tweetid");
