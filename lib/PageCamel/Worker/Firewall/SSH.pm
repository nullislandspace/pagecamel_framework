...
sub crossregister {
    ...
    $self->{loghoneylogin} = $dbh->prepare_cached("INSERT INTO ssh_honeylogins (username, pass)
                                                    VALUES (?, ?)")
            or croak($dbh->errstr);
...
sub work {
    ...
    while((my $message = $self->{clacks}->getNext())) {
        ...
        if($message->{type} eq 'disconnect') {
            ...
            if($logtext =~ /Failed\ password.*from\ (.+?)\ port\ (.*?)/i) {
            ...
            } elsif($logtext =~ /HONEY\:\ Username\ \[(.+)\]\ Password \[(.+)\]/i) {
                # Most likely an unwanted portscan. Log it as unsucessful login attempt
                my ($user, $pass) = ($1, $2);
                ...
                $reph->debuglog("*** Honeypot-Login detected ***");
            }
