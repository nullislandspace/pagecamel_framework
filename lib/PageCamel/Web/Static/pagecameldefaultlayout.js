...
$(document).ready(function() {
    ...
    $('input[type="text"],input[type="search"],textarea').each(function() {
        ...
        if(isdatefield) {
            // Add a "clear date field" button (big red X)
            var id = $(this).attr('id');
            var clearid = 'cleardatefield_' + id;

            var image = '/pics/clearfield.png';
            if(ttvars.touchinputenabled == "1") {
                image = '/pics/clearfield_large.png';
            }
            var htmlelem = '<img src="' + image + '" id="' + clearid + '" parentid="' + id + '">';
            $(this).parent().append(htmlelem);
