...
function addKeyboard(elem) {
    ...
    var keyboardConfig = {
          ...
          lockInput    : false, // Allow physical keyboard input alongside virtual keyboard

          // *** choose layout ***
          layout       : 'german-qwertz-1',
          //customLayout : { 'normal': ['{cancel}'] },
          //custom display text for buttons
          display       : {
                'bksp'  : '⌫',
                'enter' : '⏎',
                'shift' : '⇧',
          ...
          change: function(e, keyboard, el) {
                // Don't trigger additional events - the keyboard plugin handles everything internally
                // This prevents double processing of backspace and other special keys
                return;
        },
        position: {
    ...
    // Prevent native mobile keyboard from appearing while allowing physical keyboard
    elem.attr('inputmode', 'none');

    elem.keyboard(keyboardConfig);

    // Force lockInput to false AFTER initialization
    setTimeout(function() {
        var kb = elem.getkeyboard();
        if (kb && kb.options) {
            kb.options.lockInput = false;
        } else {
        }
    }, 500);
    ...
    // Use MutationObserver to watch for position changes and force correction
    var observer = null;
    var lockPosition = function() {
        var $keyboard = $('.ui-keyboard');
        if($keyboard.length > 0 && !observer) {
            observer = new MutationObserver(function(mutations) {
                mutations.forEach(function(mutation) {
                    if(mutation.type === 'attributes' && mutation.attributeName === 'style') {
                        var $kb = $(mutation.target);
                        if($kb.css('bottom') !== '0px') {
                            $kb.css('bottom', '0px');
                        }
                    }
                });
    ...
    elem.on('visible.keyboard', function() {
        positionKeyboard();
        lockPosition();
    });

    // Reposition when keyboard layout changes (e.g., shift pressed)
    elem.on('beforeVisible.keyboard change.keyboard', function() {
        setTimeout(positionKeyboard, 10);
    });
