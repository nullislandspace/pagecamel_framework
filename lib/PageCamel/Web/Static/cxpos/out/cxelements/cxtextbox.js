    ...
    _drawText() {
        ...
        if (this._text) {
            ...
            if (this._auto_line_break) {
                for (var j in text_array) {
                    ...
                    if (new_lines.length > 0) {
                        ...
                        for (var k = 0; k < new_lines.length; k++) {
                            var text_metrics = this._ctx.measureText(new_lines[k]);
                            var actualHeight = text_metrics.actualBoundingBoxAscent + text_metrics.actualBoundingBoxDescent;
                ...
                var text_metrics = this._ctx.measureText(text_array[0]);
                var actualHeight = text_metrics.actualBoundingBoxAscent + text_metrics.actualBoundingBoxDescent;
                text_lines_height += actualHeight;
                new_displaytext = text_array;
            }
            var line_height = text_lines_height / new_displaytext.length;
            var yoffs = 0;
            var start_y = this.ypixel + this.heightpixel / 2 - (text_lines_height - 1.8 * line_height) / 2;
            for (var k = 0; k < new_displaytext.length; k++) {
                var text_line = new_displaytext[k];
                var text_metrics = this._ctx.measureText(text_line);
                var actualHeight = text_metrics.actualBoundingBoxAscent + text_metrics.actualBoundingBoxDescent;
                if (new_displaytext.length == 1) {
                    start_y = this.ypixel + (this.heightpixel / 2 - actualHeight / 2) + actualHeight;
                }
                var text_x = this.xpixel + 8;
                ...
                this._ctx.fillText(text_line, text_x, start_y + yoffs, this.widthpixel);
...
//# sourceMappingURL=cxtextbox.js.map