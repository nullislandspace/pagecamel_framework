class CXDragAndDrop extends CXButton {
    constructor(ctx, x, y, width, height, is_relative = true, redraw = true) {
        ...
        this._resize = (x, y) => {
            ...
            // calculates the new center point of the dragndrop
        ...
        this._move = (x, y) => {
            ...
            var min_x = Math.min(x1, x2, x3, x4); //get the minimum x value
            var min_y = Math.min(y1, y2, y3, y4); //get the minimum y value
            var max_x = Math.max(x1, x2, x3, x4); //get the maximum x value
            var max_y = Math.max(y1, y2, y3, y4); //get the maximum y value
