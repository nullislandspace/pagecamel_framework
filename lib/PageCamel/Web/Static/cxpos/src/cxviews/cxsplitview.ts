...
//import { OrderItem } from '../cxadds/cxadds.js';
...
export class CXSplitView extends CXDefaultView {
    ...
    private _bookingTables:{left:cxa.CXTable| null, right: cxa.CXTable|null } ={left: null, right:null};
    ...
    protected _initializeSplitView(): void{
        const listx = 0.045;
        const listy = 0.15;
        const textSummHeight = 0.04;
        
        const listh = 1 - 2*listy - textSummHeight;
        const listw = 0.5 - 1.5*listx;
        const textSummWidth = listw * 0.7;
        const doredraw = true;
        this._rightList =[];  
        this._leftList =[];
        this._leftListIndex = []; 
        ...
        //create left and right dummy table
        this._bookingTables.left = Object.assign({},this.Table );
        this._bookingTables.right = new cxa.CXTable();
        ...
        /*for (let i=0; i<50; i++){
            leftorderitems.push(cxa.createOrderItem({name: `Artikel${ i }`, price:((i+1)*6.25).toFixed(2)},i, 0,i+1,false));

        } */

        let leftorderlist = cxa.createOrderList(leftorderitems);
        ...
        //arrow textbox ( left to right arrow )
        ...
        //summary textboxes
        const sumleft = new cxe.CXTextBox(this._ctx,leftList.xpos+leftList.width-textSummWidth,leftList.ypos+leftList.height,textSummWidth,textSummHeight,this.is_relative,false);
        sumleft.attributes ={...this._textbox};
        sumleft.border_width = leftList.border_width*(leftList.height/sumleft.height);
        this._elements.push(sumleft);

        const sumright = new cxe.CXTextBox(this._ctx,rightList.xpos+rightList.width-textSummWidth,rightList.ypos+rightList.height,textSummWidth,textSummHeight,this.is_relative,false);
        sumright.attributes ={...this._textbox};
        sumright.border_width=rightList.border_width*(rightList.height/sumright.height);
        this._elements.push(sumright);

        //page up and page down arrows
        const pgwidth = (sumleft.xpos-leftList.xpos)/2;
        let pgdownx = sumleft.xpos - pgwidth;
        let pgheight = sumleft.height;
        const pgdownleft = new cxe.CXArrowButton(this._ctx,pgdownx,sumleft.ypos,pgwidth,pgheight,this.is_relative,true);
        pgdownleft.arrow_direction = "down";
        pgdownleft.attributes={...this._textbox};
        pgdownleft.border_width = sumleft.border_width;
        this._elements.push(pgdownleft);

        const pgupleft = new cxe.CXArrowButton(this._ctx,pgdownleft.xpos-pgwidth,sumleft.ypos,pgwidth,pgheight,this.is_relative,true);
        pgupleft.arrow_direction = "up";
        pgupleft.attributes={...this._textbox};
        pgupleft.border_width = pgdownleft.border_width;
        this._elements.push(pgupleft);
        
        pgdownx = sumright.xpos - pgwidth;
        pgheight = sumright.height;
        const pgdownright = new cxe.CXArrowButton(this._ctx,pgdownx,sumright.ypos,pgwidth,sumright.height,this.is_relative,true);
        pgdownright.arrow_direction="down";
        pgdownright.attributes = {...this._textbox};
        pgdownright.border_width=sumright.border_width;
        this._elements.push(pgdownright);
        const pgupright = new cxe.CXArrowButton(this._ctx,pgdownright.xpos-pgwidth,sumright.ypos,pgwidth,pgheight,this.is_relative,true);
        pgupright.arrow_direction = "up";
        pgupright.attributes={...this._textbox};
        pgupright.border_width=pgdownright.border_width;
        this._elements.push(pgupright);
        ...
        const bottomline = new cxe.CXBox(this._ctx,linedistance_x,sumleft.ypos + sumleft.height +linedistance_y,this.width-2*linedistance_x,lineheight, this._is_relative, false);
        ...
        //num_field.buttons_text_block = [['1',null,'3','4'],['5','6','7','8','9','0']];
        num_field.gap = 0.01;
        ...
        num_field.setSquareSize();
        num_field.xpos=rightList.xpos + rightList.width-num_field.width;
