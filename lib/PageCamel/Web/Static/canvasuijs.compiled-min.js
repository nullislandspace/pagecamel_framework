class UIView{constructor(canvas){this.is_active=false;this.canvas=canvas;this.ctx=document.getElementById(this.canvas).getContext('2d');this.button=new UIButton();this.line=new UILine();this.text=new UIText();this.numpad=new UINumpad();this.list=new UIList();this.arrowbutton=new UIArrowButton();this.textbox=new UITextBox();this.paylist=new UIPayList();this.dragndrop=new UIDragNDrop();this.ui_types=[{type:'Button',object:this.button},{type:'Line',object:this.line},{type:'Text',object:this.text},{type:'Numpad',object:this.numpad},{type:'List',object:this.list},{type:'ArrowButton',object:this.arrowbutton},{type:'TextBox',object:this.textbox},{type:'PayList',object:this.paylist},{type:'DragNDrop',object:this.dragndrop}];this.onClick=this.onClick.bind(this);this.onMouseUp=this.onMouseUp.bind(this);this.onMouseDown=this.onMouseDown.bind(this);this.onMouseMove=this.onMouseMove.bind(this);$('#'+this.canvas).on('mousedown',this.onMouseDown);$('#'+this.canvas).on('mouseup',this.onMouseUp);$('#'+this.canvas).on('click',this.onClick);$('#'+this.canvas).on('mouseleave',this.onMouseUp);$('#'+this.canvas).on('mousemove',this.onMouseMove);}
...
onMouseMove(e){if(this.is_active==true){var canvas=$('#'+this.canvas);var x=Math.floor((e.pageX-canvas.offset().left));var y=Math.floor((e.pageY-canvas.offset().top));for(let i in this.ui_types){let ui_type=this.ui_types[i];ui_type.object.onMouseMove(x,y);}}else{return;}}}class UIText{constructor(){this.texts=[];}
add(options){this.texts.push(options);return options;}
render(ctx){for(let i in this.texts){let text=this.texts[i];ctx.font=text.font_size+'px Courier'
...
find(name){return;}}class UIButton{constructor(){this.hovering_on=null;this.buttons=[];this.mouse_down_on=null;}
add(options){this.buttons.push(options);return options;}
render(ctx){for(var i in this.buttons){var button=this.buttons[i];ctx.font=button.font_size+'px Courier';if(i==this.hovering_on){ctx.strokeStyle=button.hover_border;}
...
ctx.fillStyle=button.foreground;ctx.strokeStyle=button.foreground;if(button.displaytext){if(!button.displaytext.includes("\n")){ctx.fillText(button.displaytext,button.x+8,button.y+(button.height/2));}else{var blines=button.displaytext.split("\n");var yoffs=button.y+((button.height/2)-(9*(blines.length-1)));var j;for(j=0;j<blines.length;j++){blines[j].replace("\n",'');ctx.fillText(blines[j],button.x+8,yoffs);yoffs=yoffs+18;}}}}}
onClick(x,y){for(var i in this.buttons){var button=this.buttons[i];if(button){var startx=button.x;var starty=button.y;var endx=startx+button.width;var endy=starty+button.height;if(x>=startx&&x<=endx&&y>=starty&&y<=endy){button.callback(button.callbackData);}}}}
onMouseDown(x,y){for(var i in this.buttons){var button=this.buttons[i];var startx=button.x;var starty=button.y;var endx=startx+button.width;var endy=starty+button.height;if(x>=startx&&x<=endx&&y>=starty&&y<=endy){this.mouse_down_on=i;return;}}
return;}
onMouseUp(x,y){this.mouse_down_on=null;}
onMouseMove(x,y){for(var i in this.buttons){var button=this.buttons[i];var startx=button.x;var starty=button.y;var endx=startx+button.width;var endy=starty+button.height;if(x>=startx&&x<=endx&&y>=starty&&y<=endy){this.hovering_on=i;return;}}
this.hovering_on=null;return;}
find(name){return;}
clear(){this.mouse_down_on=null;this.hovering_on=null;this.buttons=[];}}function roundRect(ctx,x,y,w,h,radius,line_width){var r=x+w;var b=y+h;ctx.lineWidth=line_width;ctx.beginPath();ctx.moveTo(x+radius,y);ctx.lineTo(r-radius,y);ctx.quadraticCurveTo(r,y,r,y+radius);ctx.lineTo(r,y+h-radius);ctx.quadraticCurveTo(r,b,r-radius,b);ctx.lineTo(x+radius,b);ctx.quadraticCurveTo(x,b,x,b-radius);ctx.lineTo(x,y+radius);ctx.quadraticCurveTo(x,y,x+radius,y);ctx.fill();ctx.stroke();}
class UILine{constructor(){this.lines=[];}
...
onMouseUp(x,y){return;}
onMouseMove(x,y){return;}
find(name){return;}}class UIList{constructor(){this.lists=[]
...
find(name){for(var i in this.lists){var list=this.lists[i];if(list.name==name){return list;}}}}class UINumpad{constructor(){this.button=new UIButton();this.numpads=[];}
add(options){this.numpads.push(options);var keyvalues=[['+/-','ZWS','BS'],['7','8','9'],['4','5','6'],['1','2','3'],['x','0',',']];if(!options.show_keys.x){keyvalues[4][0]=null;}
...
find(name){return;}}class UIArrowButton{constructor(){this.arrowbuttons=[];this.button=new UIButton();}
...
onMouseMove(x,y){this.button.onMouseMove(x,y);}
find(name){return;}
clear(){this.arrowbuttons=[];this.button.clear();}}class UITextBox{constructor(){this.textboxes=[];}
add(options){options.setText=(params)=>{options.displaytext=params;}
this.textboxes.push(options);return options;}
render(ctx){for(var i in this.textboxes){var textbox=this.textboxes[i];ctx.font=textbox.font_size+'px Courier';ctx.strokeStyle=textbox.border;var grd;if(textbox.grd_type=='horizontal'){grd=ctx.createLinearGradient(textbox.x,textbox.y,textbox.x+textbox.width,textbox.y);}
...
if(!textbox.border_radius){ctx.fillRect(textbox.x,textbox.y,textbox.width,textbox.height);ctx.strokeRect(textbox.x,textbox.y,textbox.width,textbox.height);}else{roundRect(ctx,textbox.x,textbox.y,textbox.width,textbox.height,textbox.border_radius,textbox.border_width);}
ctx.fillStyle=textbox.foreground;ctx.strokeStyle=textbox.foreground;if(textbox.displaytext){if(!textbox.displaytext.includes("\n")){if(textbox.align=='right'){var text_width=ctx.measureText(textbox.displaytext).width;ctx.fillText(textbox.displaytext,textbox.x+textbox.width-text_width-8,textbox.y+(textbox.height/2)+textbox.font_size/3.3);}
else{ctx.fillText(textbox.displaytext,textbox.x+8,textbox.y+(textbox.height/2)+textbox.font_size/3.3);}}else{var blines=textbox.displaytext.split("\n");var yoffs=textbox.y+((textbox.height/2)-(9*(blines.length-1)));for(var j=0;j<blines.length;j++){if(textbox.align=='right'){blines[j].replace("\n",'');var text_width=ctx.measureText(blines[j]).width;ctx.fillText(blines[j],textbox.x+textbox.width-text_width-8,yoffs);}
...
clear(){this.textboxes=[];}}class UIPayList{constructor(){this.arrowbutton=new UIArrowButton();this.paylists=[];this.listitem=new UIListItem();}
add(options){options.scrollposition=0;options.max_paylist_items=0;options.scrollbarsize=options.height-options.pagescrollbuttonheight-2*options.scrollbarwidth-options.border_width;options.scrollbar_y=options.scrollbarwidth+options.border_width;options.mousedown_scrollbar_y=null;options.setList=(params)=>{options.list=params;this.update()
return;}
...
return;}
options.scrollup=(params)=>{if(options.scrollposition>0){options.scrollposition-=1;this.update();}
return;}
options.scrolldown=(params)=>{var nextitem=options.scrollposition+options.max_paylist_items+1;if(nextitem<=options.list.length){options.scrollposition+=1;this.update();}
return;}
...
this.arrowbutton.add({x:options.x+options.width-options.scrollbarwidth,y:options.y,width:options.scrollbarwidth,height:options.scrollbarwidth,direction:'up',background:['#ffffff'],foreground:'#000000',border:'#000000',border_width:options.border_width,hover_border:'#000000',callback:options.scrollup});this.arrowbutton.add({x:options.x+options.width-options.scrollbarwidth,y:options.y+options.height-options.scrollbarwidth-options.pagescrollbuttonheight,width:options.scrollbarwidth,height:options.scrollbarwidth,direction:'down',background:['#ffffff'],foreground:'#000000',border:'#000000',border_width:options.border_width,hover_border:'#000000',callback:options.scrolldown});this.arrowbutton.add({x:options.x,y:options.y+options.height-options.pagescrollbuttonheight+5,width:options.width/2-2,height:options.pagescrollbuttonheight,direction:'up',background:['#ffffff'],foreground:'#000000',border:'#000000',border_width:options.border_width,hover_border:'#000000',callback:options.previousPage});this.arrowbutton.add({x:options.x+options.width/2+2,y:options.y+options.height-options.pagescrollbuttonheight+5,width:options.width/2-2,height:options.pagescrollbuttonheight,direction:'down',background:['#ffffff'],foreground:'#000000',border:'#000000',border_width:options.border_width,hover_border:'#000000',callback:options.nextPage});this.paylists.push(options);return options;}
update(){this.listitem.clear()
for(var i in this.paylists){var paylist=this.paylists[i];var x=paylist.x;var height=paylist.elementOptions.height;var font_size=paylist.elementOptions.font_size;var selectedBackground=paylist.elementOptions.selectedBackground;var foreground=paylist.foreground;var width=paylist.width-paylist.scrollbarwidth;paylist.max_paylist_items=Math.round((paylist.height-(paylist.pagescrollbuttonheight+5))/height-0.49);var max_scrollbarheight=paylist.height-paylist.pagescrollbuttonheight-2*paylist.scrollbarwidth-paylist.border_width;paylist.scrollbarsize=this.getScrollbarSize(paylist.max_paylist_items,paylist.list.length)*max_scrollbarheight;paylist.scrollbar_y=(max_scrollbarheight-paylist.scrollbarsize)*(paylist.scrollposition/(paylist.list.length-paylist.max_paylist_items))+paylist.scrollbarwidth+paylist.border_width/2;for(var j in paylist.list){var index=j-paylist.scrollposition;if(index<paylist.max_paylist_items&&index>=0){var item=paylist.list[j];var y=paylist.y+paylist.elementOptions.height*index;this.listitem.add({...{x:x,y:y,width:width,height:height,font_size:font_size,selected:paylist.selectedID,border_width:paylist.border_width,selectedBackground:selectedBackground,foreground:foreground,callback:paylist.setSelected,id:j},...item});}}}}
...
return;}}
return;}
onMouseUp(x,y){this.arrowbutton.onMouseUp(x,y);this.listitem.onMouseUp(x,y);for(i in this.paylists){var paylist=this.paylists[i];paylist.mousedown_scrollbar_y=null;}
return;}
...
return;}
find(name){for(var i in this.paylists){var paylist=this.paylists[i];if(paylist.name==name){return paylist;}}}}class UIListItem{constructor(){this.listitems=[];}
...
onClick(x,y){for(var i in this.listitems){var listitem=this.listitems[i];var startx=listitem.x+listitem.border_width/1.8;var starty=listitem.y+listitem.border_width/2;var endx=listitem.width-listitem.border_width*1.2+startx;var endy=starty+listitem.height;if(x>=startx&&x<=endx&&y>=starty&&y<=endy){listitem.callback(listitem.id);}}}
onMouseDown(x,y){return;}
onMouseUp(x,y){return;}
onMouseMove(x,y){return;}
find(name){return;}
clear(){this.listitems=[];}}class UIDragNDrop{constructor(){this.textbox=new UITextBox()
this.dragndrops=[];this.mouse_down=false;this.mouse_down_x=null;this.mouse_down_y=null;}
add(options){this.dragndrops.push(options);this.textbox.add(options);return options;}
render(ctx){this.textbox.render(ctx);}
onClick(x,y){return;}
onMouseDown(x,y){for(var i=this.dragndrops.length-1;i>=0;i--){var dragndrop=this.dragndrops[i];var startx=dragndrop.x;var starty=dragndrop.y;var endx=startx+dragndrop.width;var endy=starty+dragndrop.height;if(x>=startx&&x<=endx&&y>=starty&&y<=endy){this.mouse_down=true;console.log(i);this.array_move(this.dragndrops,i,this.dragndrops.length-1);this.textbox.textboxes=this.dragndrops;this.mouse_down_x=x-startx;this.mouse_down_y=y-starty;return;}}
return;}
onMouseUp(x,y){this.mouse_down=false;this.mouse_down_x=null;this.mouse_down_y=null;}
array_move(arr,old_index,new_index){if(new_index>=arr.length){var k=new_index-arr.length+1;while(k--){arr.push(undefined);}}
arr.splice(new_index,0,arr.splice(old_index,1)[0]);return arr;};onMouseMove(x,y){if(this.mouse_down){var dragndrop=this.dragndrops[this.dragndrops.length-1];var new_x=x-this.mouse_down_x;var new_y=y-this.mouse_down_y;dragndrop.x=new_x;dragndrop.y=new_y;}
return;}
find(name){return;}}