class UIDragNDrop {
    ...
    render(ctx) {
        ...
        for (var i in this.dragndrops) {
            ...
            if (dragndrop.editable == true && dragndrop.selected == true) {
                ...
                ctx.lineWidth = 5;
                ctx.moveTo(dragndrop.center_x - 5, dragndrop.center_y - 5);
                ctx.lineTo(dragndrop.center_x + 5, dragndrop.center_y + 5);
                ctx.stroke();
            }
        }
    ...
    calculateCornerPositions(x, y, width, height, angle) {
        var [corners_x, corners_y] = this.calculateUnsortedCornerPositions(x, y, width, height, angle);
        //retruns sorted corner positions
        return [corners_x.sort(function (a, b) { return a - b }), corners_y.sort(function (a, b) { return a - b })];
    }
    calculateUnsortedCornerPositions(x, y, width, height, angle) {
        ...
        var corners_y = [rotate(center_x, center_y, x + width, y + height, angle)[1], rotate(center_x, center_y, x, y, angle)[1],
        rotate(center_x, center_y, x + width, y, angle)[1], rotate(center_x, center_y, x, y + height, angle)[1]];
        ...
        if (this.mouse_down) {
            ...
            if (dragndrop.mouse_down_action != '' && dragndrop.editable == true && dragndrop.selected) {
                ...
                if (dragndrop.mouse_down_action == 's-resize' || dragndrop.mouse_down_action == 'se-resize' || dragndrop.mouse_down_action == 'sw-resize') {
                    ...
                    var [x_corners, y_corners] = this.calculateCornerPositions(new_x, new_y, dragndrop.width, dragndrop.height, dragndrop.angle);
                    ...
                    console.log(dragndrop.angle);
                    if (max_x_corner >= dragndrop.contain_x + dragndrop.contain_width - 1 && dragndrop.height <= new_height && Math.abs(dragndrop.angle) > 0 && Math.abs(dragndrop.angle) < 180) {
                        var offset = max_x_corner - center_x - dragndrop.height / 2;
                        var new_location = dragndrop.contain_x + dragndrop.contain_width - dragndrop.height - offset;
                        new_height = dragndrop.contain_x + dragndrop.contain_width - new_location - offset;

                    }
                    if (min_x_corner <= dragndrop.contain_x + 1 && dragndrop.height <= new_height && Math.abs(dragndrop.angle) < 360 && Math.abs(dragndrop.angle) > 180) {
                        var offset = center_x - min_x_corner - dragndrop.height / 2;
                        //var new_location = dragndrop.contain_x + dragndrop.height - offset;
                        new_height = dragndrop.height + (new_x - dragndrop.contain_x - offset);
                    }
                    /*if (max_y_corner >= dragndrop.contain_y + dragndrop.contain_height - 1 && dragndrop.height <= new_height && (Math.abs(dragndrop.angle) > 270 || Math.abs(dragndrop.angle) < 90)) {
                        var offset = max_y_corner - center_y - dragndrop.height / 2;
                        new_height = dragndrop.contain_y + dragndrop.contain_height - new_y - offset;
                    }
                    if (min_y_corner <= dragndrop.contain_y + 1 && dragndrop.height <= new_height && Math.abs(dragndrop.angle) < 270 && Math.abs(dragndrop.angle) > 90) {
                        var offset = center_y - min_y_corner - dragndrop.height / 2;
                        new_height = dragndrop.height + (new_y - dragndrop.contain_y - offset);
                    } */
                ...
                /*var [corners_x, corners_y] = this.calculateCornerPositions(new_x, new_y, new_height, new_width, dragndrop.angle);

                var [u_corners_x, u_corners_y] = this.calculateUnsortedCornerPositions(new_x, new_y, new_height, new_width, dragndrop.angle);
                var min_x_corner = null
                console.log(u_corners_x, u_corners_y)
                for (var i in u_corners_x) {
                    var u_corner_x = u_corners_x[i];
                    if (min_x_corner == null || u_corner_x < min_x_corner) {
                        min_x_corner = [u_corner_x, u_corners_y[i]];
                    }
                }*/
                //console.log(min_x_corner);
                /*var center_x = (x_corners[0] + x_corners[1] + x_corners[2] + x_corners[3]) / 4
                var center_y = (y_corners[0] + y_corners[1] + y_corners[2] + y_corners[3]) / 4
                var rect_container_x = x_corners[0];
                var rect_container_width = x_corners[3] - x_corners[0];
                var rect_container_y = y_corners[0];
                var rect_container_heigt = y_corners[3] - y_corners[0];

                if (rect_container_x < dragndrop.contain_x) {
                    console.log(Math.abs(dragndrop.angle));
                } */
                /*if (min_x_corner < dragndrop.contain_x) {
                    console.log('outside');
                    var offset = center_x - min_x_corner - dragndrop.width / 2;
                    new_x = offset + dragndrop.contain_x;
                    new_width = dragndrop.width + (dragndrop.x - dragndrop.contain_x - offset);
                }*/
