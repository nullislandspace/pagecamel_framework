class PaymentView extends UIView {
    ...
    addToInvoice = (article) => {
        ...
        invoice_list.push({
            ...
            lineitem: [
                ...
                { location: 0.95, align: 'left', displaytext: String(article.article_price * qty) },
            ]
        });
    ...
    createElements = (mainview, editarticlesview) => {
        ...
        this.addElement('Button', {
            ...
            callback: () => {
                editarticlesview.setActive(true);
                for (var menu of this.menu_list) {
                    if (this.selectedMenu == menu.menu_name) {
                        editarticlesview.setSelectedMenu(menu);
                        this.setActive(false);
                        return;
                    }
                }
                ...
                if (invoices.length > 0) {
                    try {
                        for (var article of invoices) {
                            price_sum += parseFloat(article.lineitem[3].displaytext);
                        }
                    }
                    catch {
                        return;
            ...
            for (var article of invoice.articles) {
                this.show_invoices[this.show_invoices.length - 1].invoice.push({
                    ...
                    lineitem: [
                        ...
                        { location: 0.95, align: 'left', displaytext: String(article.article.article_price * article.qty) },
                    ]
                });
            }
