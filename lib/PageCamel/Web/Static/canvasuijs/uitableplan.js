class UITablePlan {
    ...
    add(options) {
        ...
        options.list.add({
            name: 'rooms',
            x: options.x + 10, y: options.y + options.height - this.bottom_bar_height + 10, width: options.width - 225 - 20, height: 75, font_size: 25, scrollbarwidth: 34,
        ...
        for (var i = 0; i < 50; i++) {
            test_list.push(
                {
                    displaytext: 'Button' + i,
                    background: ['#c9f7c8', '#10ff0c'], foreground: '#000000', border: '#10ff0c', border_width: 3, grd_type: 'vertical',
                    hover_border: '#ffffff',
                    callback: buttonSendMessage,
                    callbackData: { key: "ZAHLUNGSART_SELECTREST", value: i },

                })
        }
        options.list.find('rooms').setList(test_list);
        options.selected = null;
        options.group_id = this.uitableplans.length
        if (options.active === undefined) {
            options.active = true;
        }
        options.setActive = (state) => {
            options.active = state;
            options.dragndrop.setActiveByGroupID(options.group_id, state);
            if (state) {
                options.getSQLData();
            }
            this.update();
        ...
        options.edit = (group_id) => {
            ...
            options.dragndrop.setEditable(group_id, true);
            ...
            for (var i in options.dragndrop.dragndrops) {
                ...
                if (dragndrop.group_id == group_id) {
                    ...
                    options.dragndrop.changeHandler(dragndrop);
                    return;
                }
            }
        ...
        options.save = (group_id) => {
            options.dragndrop.setEditable(group_id, false);
            options.setSQLData();
            options.select = false;
            options.editable = false;
            options.circle = false;
        ...
        options.setSQLData = () => {
            ...
            executeSQL("DELETE FROM tableplan WHERE id=?", options.name);
            var data = [] //get current dragndrop data
            for (var i in options.dragndrop.dragndrops) {
                var dragndrop = options.dragndrop.dragndrops[i];
                if (dragndrop.group_id == options.group_id) {
                    data.push({ ...dragndrop });
                    var j = data.length - 1;
                    data[j].x = (data[j].x - options.x) / (options.width - 225);
                    data[j].y = (data[j].y - options.y) / (options.height - this.bottom_bar_height);
                    data[j].width = data[j].width / (options.width - 225);
                    data[j].height = data[j].height / (options.height - this.bottom_bar_height);
                }
            }
            ...
            if (options.active) {
                ...
                if (data.length > 0) {
                    for (var i in data) {
                        ...
                        data[i].active = options.active;
                        //convert relative positions
                        data[i].x = data[i].x * (options.width - 225) + options.x;
                        data[i].y = data[i].y * (options.height - this.bottom_bar_height) + options.y;
                        data[i].width = data[i].width * (options.width - 225);
                        data[i].height = data[i].height * (options.height - this.bottom_bar_height);
                        data[i].contain_height = options.height - this.bottom_bar_height;
                        data[i].contain_width = options.width - 225;
                        data[i].contain_x = options.x;
                        data[i].contain_y = options.y;
                    }
                    options.dragndrop.loadSaved(options.group_id, data);
                }
            }
        }
        ...
        options.undo = (group_id) => {
            if (options.undoable.length > 1) {
                options.dragndrop.replaceElementsByGropID(group_id, options.undoable[options.undoable.length - 2]);
        ...
        options.redo = (group_id) => {
            if (options.redoable.length > 0) {
                options.dragndrop.replaceElementsByGropID(group_id, options.redoable[options.redoable.length - 1]);
        ...
        options.deleteSelected = (group_id) => {
            options.selected = null;
            options.dragndrop.deleteSelected(group_id);
            this.update();
        }
        options.getSQLData();
    ...
    onClick(x, y) {
        ...
        for (var i in this.uitableplans) {
            this.uitableplans[i].dragndrop.onClick(x, y);
            this.uitableplans[i].list.onClick(x, y);
        }
    ...
    onMouseDown(x, y) {
        for (var i in this.uitableplans) {
            this.uitableplans[i].list.onMouseDown(x, y);
            var uitableplan = this.uitableplans[i];
    ...
    onMouseUp(x, y) {
                ...
                if (uitableplan.editable == true && (uitableplan.circle == true || uitableplan.rect == true) && uitableplan.draw.mousedown_x != null) {
                    ...
                    if (uitableplan.rect) {
                        uitableplan.dragndrop.add({
                            displaytext: '', group_id: i, contain_x: uitableplan.x, contain_y: uitableplan.y, contain_width: uitableplan.width - 225, contain_height: uitableplan.height - this.bottom_bar_height,
                            background: ['#31A2F2'], foreground: '#000000', border: '#0579ff', border_width: 0, grd_type: 'vertical', editable: true, active: uitableplan.active,
                        ...
                        uitableplan.dragndrop.add({
                            displaytext: '', group_id: i, contain_x: uitableplan.x, contain_y: uitableplan.y, contain_width: uitableplan.width - 225, contain_height: uitableplan.height - this.bottom_bar_height,
                            background: ['#31A2F2'], foreground: '#000000', border: '#0579ff', border_width: 0, grd_type: 'vertical', editable: true, type: 'circle', active: uitableplan.active,
                            x: x, y: y, width: width, height: height, font_size: 25, angle: 0, addToUndoable: uitableplan.addToUndoable, isSelected: uitableplan.isSelected, callback: uitableplan.tableClicked,
                        });
                    }
                    uitableplan.draw.mousedown_x = null;
        ...
        for (var i in this.uitableplans) {
            this.uitableplans[i].dragndrop.onMouseUp(x, y);
            this.uitableplans[i].list.onMouseUp(x, y);
    ...
    onMouseMove(x, y) {
        ...
        for (var i in this.uitableplans) {
            this.uitableplans[i].list.onMouseMove(x, y);
    ...
    clear() {
        ...
        this.textbox.clear();
        for (var i in this.uitableplans) {
            this.uitableplans[i].dragndrop.clear();
            this.uitableplans[i].list.clear();
        }
    }
}
