class UITablePlan {
        ...
        for (var i in this.uitableplans) {
            ...
            if (uitableplan.editable == false) {
                ...
                for (var j in uitableplan.rects) {
                    ...
                    this.dragndrop.add({
                        displaytext: 'Hello World', name: 'tbox',
                        background: ['#1f1f1f', '#10ff0c'], foreground: '#000000', border: '#10ff0c', border_width: 3, grd_type: 'vertical',
                        x: rect.x, y: rect.y, width: rect.width, height: rect.height, font_size: 15, align: 'right', callbackData: { index: j },
        ...
        for (var i in this.uitableplans) {
            ...
            if (uitableplan.editable == true && (uitableplan.circle == true || uitableplan.rect == true) && uitableplan.draw.mousedown_x != null) {
                uitableplan.rects.push({
                    x: uitableplan.draw.mousedown_x + uitableplan.x, y: uitableplan.draw.mousedown_y + uitableplan.y,
                    width: uitableplan.draw.mousemove_x - uitableplan.draw.mousedown_x, height: uitableplan.draw.mousemove_y - uitableplan.draw.mousedown_y
                });
                uitableplan.draw.mousedown_x = null;
                uitableplan.draw.mousedown_y = null;
    ...
    onMouseMove(x, y) {
        for (var i in this.uitableplans) {
            var uitableplan = this.uitableplans[i];
            if (uitableplan.editable == true && (uitableplan.draw.circle == true || uitableplan.rect == true)) {
                if (x > uitableplan.x && x < uitableplan.x + uitableplan.width - 225 && y > uitableplan.y && y < uitableplan.y + uitableplan.height - 70) {
                    uitableplan.draw.mousemove_x = x - uitableplan.x;
                    uitableplan.draw.mousemove_y = y - uitableplan.y;
                    triggerRepaint();
