    ...
    add(options) {
        ...
        options.setSQLData = () => {
            ...
            for (var i in sorted_rooms) {
                ...
                room_data.push({ name: room.displaytext, tables: [] })
                for (var j in options.dragndrop.dragndrops) {
                    var dragndrop = options.dragndrop.dragndrops[j];
            ...
            if (options.active) {
                ...
                if (room_data.length > 0) {
                    for (var i in room_data) {
                        options.room_displaytexts.push({ displaytext: room_data[i].name });
                    ...
                    console.log('Displaytexts:', options.room_displaytexts);
                    options.dragndrop.loadSaved(options.active_room, tables);
                    options.buttonrow.find('rooms').setList(options.room_displaytexts);
                }
            }
        }
