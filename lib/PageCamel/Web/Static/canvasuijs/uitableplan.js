class UITablePlan {
    ...
    add(options) {
        ...
        options.setActive = (state) => {
            ...
            this.dragndrop.setActiveByGroupID(options.group_id, state);
            if (state) {
                options.getSQLData();
            }
            this.update();
        ...
        options.setTableActive = (number, state) => {
            for (var j in this.dragndrop.dragndrops) {
                var dragndrop = this.dragndrop.dragndrops[j];
        ...
        options.duplicate = () => {
            if (options.selected != null) {
                this.dragndrop.add({ ...options.selected });
                options.selected.selected = false;
                options.selected = null;
                options.isSelected(this.dragndrop.dragndrops[this.dragndrop.dragndrops.length - 1]);
                options.selected.selected = true;
                options.selected.changed = true;
                this.dragndrop.changeHandler(options.selected);
            }
        }
        ...
        options.setList = (unixTime, data) => {
            ...
            if (typeof tableline === 'undefined' || typeof tableline[0] === 'undefined') {
                needtableupdate = 1;
            } else {
                var tabletimestamp = tableline[0].values[0][1];
        ...
        options.setSQLData = () => {
            ...
            for (var i in this.dragndrop.dragndrops) {
                ...
                if (dragndrop.group_id == options.group_id) {
                    ...
                    data[j].y = (data[j].y - options.y) / (options.height - 70);
                    data[j].width = data[j].width / (options.width - 225);
                    data[j].height = data[j].height / (options.height - 70);
                }
            }
        ...
        options.getSQLData = () => {
            if (options.active) {
                ...
                if (data.length > 0) {
                    ...
                    console.log('Group_id: ' + options.group_id);
