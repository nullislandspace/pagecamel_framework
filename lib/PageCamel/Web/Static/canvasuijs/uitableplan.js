    ...
    add(options) {
        ...
        options.isSelected = (object_selected) => {
            ...
            if (options.selected != null) {
                var obj = options.textbox.find(options.selected.active_room);
        ...
        options.setRoomActive = (room) => {
            options.dragndrop.setActiveByGroupID(options.active_room, false);
            options.active_room = room;
            options.dragndrop.setActiveByGroupID(options.active_room, true);
        ...
        options.buttonrow.add({
            ...
            contain_height: options.height - this.bottom_bar_height,
        ...
        options.setActive = (state) => {
            ...
            options.dragndrop.setActiveByGroupID(options.active_room, state);
            if (state) {
                options.getSQLData();
            }
            this.update();
        ...
        options.edit = () => {
            ...
            for (var i in options.dragndrop.dragndrops) {
                ...
                if (dragndrop.group_id == options.active_room) {
        ...
        options.numberInput = (val) => {
            var obj = options.textbox.find(options.active_room);
        ...
        options.getSQLData = () => {
            if (options.active) {
                ...
                if (data.length > 0) {
                    ...
                    options.dragndrop.loadSaved(options.active_room, data);
                }
            }
        }
        ...
        options.undo = () => {
            if (options.undoable.length > 1) {
                options.dragndrop.replaceElementsByGropID(options.active_room, options.undoable[options.undoable.length - 2]);
        ...
        options.redo = () => {
            if (options.redoable.length > 0) {
                options.dragndrop.replaceElementsByGropID(options.active_room, options.redoable[options.redoable.length - 1]);
                    ...
                    if (uitableplan.editable == true && (uitableplan.circle == true || uitableplan.rect == true) && uitableplan.draw.mousedown_x != null) {
                        ...
                        if (uitableplan.rect) {
                            uitableplan.dragndrop.add({
                                displaytext: '', active_room: i, contain_x: uitableplan.x, contain_y: uitableplan.y, contain_width: uitableplan.width - 225, contain_height: uitableplan.height - this.bottom_bar_height,
