            ...
            if (uitableplan.active) {
                if (uitableplan.editable == false) {
                    ...
                    if (uitableplan.selected != null) {
                        ...
                        console.log(uitableplan.selected.x);
                        uitableplan.textinput.add({
                            displaytext: String(Math.round(uitableplan.selected.x - uitableplan.x)), name: 'posx', type: 'text',
                            background: ['#ffffff'], foreground: '#000000', border: '#000000', border_width: 3,
                            x: uitableplan.x + uitableplan.width - 225 - 350, y: uitableplan.y + uitableplan.height - this.bottom_bar_height + 10,
                            width: 190, height: 50, font_size: 30, align: 'left',
                            callback: (posx) => {
                                ...
                                //var offset_y = center_y - min_y_corner - dragndrop.height / 2;
                                console.log('posx', posx);
                                console.log('xCorner', min_x_corner);
                                console.log('width:', uitableplan.selected.width);
                                console.log('Center:', center_x);
                                console.log('offset:', offset_x)

                                if (posx !== '' && parseInt(posx) + uitableplan.x <= uitableplan.x + uitableplan.width - 225 - uitableplan.selected.width - offset_x) {
                                    uitableplan.selected.x = parseInt(posx) + uitableplan.x + offset_x;
                                    triggerRepaint();
                                }
                                else if (posx === '') {
                                    uitableplan.selected.x = uitableplan.x + offset_x;
                                    uitableplan.textinput.find('posx').setText(String(Math.round(offset_x)));
                                }
                                else if (parseInt(posx) + uitableplan.x > uitableplan.x + uitableplan.width - 225 - uitableplan.selected.width - offset_x) {
    ...
    render(ctx) {
        for (var i in this.uitableplans) {
            ...
            if (uitableplan.active == true) {
                ...
                //console.log(uitableplan.background_imgs);
                if (uitableplan.background_imgs[uitableplan.active_room] !== undefined) {
            ...
            if (uitableplan.active) {
                ...
                if (!uitableplan.inputDisabled) {
                    ...
                    if (uitableplan.selected) {
                        if (Math.round(uitableplan.selected.x - uitableplan.x) >= 0) {
                            uitableplan.textinput.find('posx').setText(String(Math.round(uitableplan.selected.x - uitableplan.x)));
                        }
                    }
