class UITablePlan {
    ...
    add(options) {
        ...
        options.setRoomActive = (room_number) => {
        ...
        options.setSQLData = () => {
            ...
            if (sorted_rooms.length == 0 && options.dragndrop.dragndrops.length > 0) {
                options.buttonrow.find('rooms').setList([{ background: ['#31A2F2'], displaytext: "", foreground: "#000000", i: 0 }]);
        ...
        options.getSQLData = () => {
            if (options.active) {
                ...
                if (room_data.length > 0) {
                    ...
                    options.dragndrop.loadSaved(options.active_room, tables);
        ...
        options.undo = () => {
            if (options.undoable[options.active_room] != undefined) {
                if (options.undoable[options.active_room].length > 1) {
                    options.dragndrop.replaceElementsByGropID(options.active_room, options.undoable[options.active_room][options.undoable[options.active_room].length - 2]);
        ...
        options.redo = () => {
            if (options.redoable[options.active_room] != undefined) {
                if (options.redoable[options.active_room].length > 0) {
                    options.dragndrop.replaceElementsByGropID(options.active_room, options.redoable[options.active_room][options.redoable[options.active_room].length - 1]);
        ...
        for (var i in this.uitableplans) {
            ...
            if (uitableplan.active) {
                if (uitableplan.editable == false) {
                    ...
                    if (uitableplan.is_room_selected != false) {
                        uitableplan.button.add({
                    ...
                    if (uitableplan.selected != null || uitableplan.is_room_selected != false) {
                        //color Selector
                        uitableplan.button.add({
                            displaytext: "",
