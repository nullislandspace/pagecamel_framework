class UITablePlan {
    ...
    add(options) {
        options.table_exists_warning = false;
        ...
        options.text = new UIText(this.canvas);
        ...
        options.isSelected = (object_selected) => {
            ...
            if (options.selected != null) {
                ...
                options.table_exists_warning = false;
                console.log('Set Text:', options.selected);
        ...
        options.duplicate = () => {
            if (options.selected != null) {
                ...
                if (options.selected.type != 'text' && options.selected.displaytext != '') {
                    if (!options.TableExists(parseInt(options.selected.displaytext) + 1)) {
                        options.selected.displaytext = String(parseInt(options.selected.displaytext) + 1);
                    }
                    else {
                        options.selected.displaytext = '';
                    }
                }
                ...
                options.textinput.find('tablename').setText(options.selected.displaytext);
        ...
        options.TableExists = (number) => {
            for (var table of options.dragndrop.dragndrops) {
                if (parseInt(table.displaytext) == number && table.type != 'text') {
                    return true;
                }
            }
            return false;
        ...
        options.numberInput = (val) => {
            var obj = options.textinput.find('tablename');
            var new_text = obj.getText();
            if (obj.type == 'number') {
                if (!options.TableExists(parseInt(new_text))) {
                    options.table_exists_warning = false;
                    options.selected.displaytext = new_text;
                    options.selected.change();
                }
                else {
                    options.table_exists_warning = true;
                }
            }
            else {
                options.selected.displaytext = new_text;
        ...
        options.numberInputTableSelect = (val) => {
            ...
            if (val.value >= 0 || val.value == ',') {
                ...
                obj.cursorPos = obj.getText().length;
            }
            else if (val.value == '⌫') {
    ...
    warningUpdate() {
        for (var i in this.uitableplans) {
            ...
            uitableplan.text.clear();
            if (uitableplan.active) {
                if (uitableplan.table_exists_warning && uitableplan.selected != null) {
                    uitableplan.text.add({
                        displaytext: _trquote('Table allready exists'), x: uitableplan.x + uitableplan.width - 225 + 20,
                        y: uitableplan.y + 100, font_size: 12, background: ['#ff0000'],
                    });
                }
            }
        }
                ...
                if (uitableplan.editable == false) {
                    ...
                    if (uitableplan.selected != null) {
                        if (uitableplan.selected.type != 'text') {
                            uitableplan.textinput.add({
                                displaytext: '', name: 'tablename', label: _trquote('Table Nr.'),
                                background: ['#ffffff'], foreground: '#000000', border: '#000000', border_width: 3,
                    ...
                    if (uitableplan.selected != null) {
                        ...
                        if (uitableplan.selected.type != 'image') {
                            uitableplan.button.add({
                                ...
                                background: ['#4fbcff', '#009dff'], foreground: '#000000', border: '#4fbcff', border_width: 3, grd_type: 'vertical',
                                x: uitableplan.x + uitableplan.width - 225 + 10, y: uitableplan.y + 110, width: 205, height: 50, border_radius: 10, font_size: 20, hover_border: '#009dff',
                                callback: () => {
                            ...
                            uitableplan.textinput.add({
                                ...
                                background: ['#ffffff'], foreground: '#000000', border: '#000000', border_width: 3,
                                x: uitableplan.x + uitableplan.width - 225 + 135, y: uitableplan.y + uitableplan.height - this.bottom_bar_height - 30, width: 80, height: 35, font_size: 21, align: 'left',
                        ...
                        uitableplan.button.add({
                            ...
                            background: ['#4fbcff', '#009dff'], foreground: '#000000', border: '#4fbcff', border_width: 3, grd_type: 'vertical',
                            x: uitableplan.x + uitableplan.width - 225 + 10, y: uitableplan.y + 170, width: 205, height: 50, border_radius: 10, font_size: 20, hover_border: '#009dff',
                            callback: () => {
                                uitableplan.change_background_color = false;
                        ...
                        uitableplan.textinput.add({
                            ...
                            background: ['#ffffff'], foreground: '#000000', border: '#000000', border_width: 3,
                            x: uitableplan.x + uitableplan.width - 225 + 135, y: uitableplan.y + uitableplan.height - this.bottom_bar_height - 70, width: 80, height: 35, font_size: 21, align: 'left',
                        ...
                        if (uitableplan.change_background_color || uitableplan.change_frame_color) {
                            uitableplan.colorpalet.add({
                                x: uitableplan.x + uitableplan.width - 225 + 10, y: uitableplan.y + 230, width: 205, height: uitableplan.height - this.bottom_bar_height - 300, callback: uitableplan.colorInput,
                            });
                        }
                    }
            ...
            if (!uitableplan.inputDisabled) {
                ...
                if (uitableplan.editable == true && (uitableplan.circle == true || uitableplan.rect == true || uitableplan.image == true || uitableplan.text_label == true)) {
                    if (x > uitableplan.x && x < uitableplan.x + uitableplan.width - 225 && y > uitableplan.y && y < uitableplan.y + uitableplan.height - this.bottom_bar_height) {
                        ...
                        this.update();
                        if (uitableplan.selected != null) {
                            var obj = uitableplan.textinput.find('tablename');
                            uitableplan.table_exists_warning = false;
                            obj.setText(uitableplan.selected.displaytext);
                        }
                    }
                }
                else {
