class CanvasUIJs {
    ...
    addLateType = (typename, classname) => {
        this.lateregister.push({ typename: typename, classname: classname });
    }
    getTypes = () => {
        var ui_types = [];
        ...
        for (var type of this.lateregister) {
            var object = new type.classname(this.canvas);
            ui_types.push({ type: type.typename, object: object });
        }
        return ui_types;
...
class UIView {
    constructor(canvas) {
        ...
        $("#upload").on("change", this.onFileInput);
        /*this.d_options = {
            background-color: #...
        }*/
    }
    ...
    onClick(e) {
        ...
        if (this.is_active == true) {
            ...
            for (let i in this.ui_types) {
                ...
                if (ui_type.object.onClick != undefined) {
                    if (!dialog_active || ui_type.type == 'Dialog') {
                        if (document.fullscreenElement && document.fullscreenElement.id === this._canvas) {
                            ui_type.object.onClick(x * window.devicePixelRatio, y * window.devicePixelRatio);
                        }
                        else {
            ...
            //document.getElementById('poscanvas').requestFullscreen();
        } else {
            return;
        }
        ...
        if (this.is_active == true) {
            ...
            for (let i in this.ui_types) {
                ...
                if (ui_type.object.onMouseUp != undefined) {
                    if (!dialog_active || ui_type.type == 'Dialog') {
                        if (document.fullscreenElement && document.fullscreenElement.id === this._canvas) {
                            ui_type.object.onMouseUp(x * window.devicePixelRatio, y * window.devicePixelRatio);
                        }
                        else {
        ...
        if (this.is_active == true) {
            ...
            for (let i in this.ui_types) {
                ...
                if (ui_type.object.onMouseDown != undefined) {
                    if (!dialog_active || ui_type.type == 'Dialog') {
                        if (document.fullscreenElement && document.fullscreenElement.id === this._canvas) {
                            ui_type.object.onMouseDown(x * window.devicePixelRatio, y * window.devicePixelRatio);
                        }
                        else {
        ...
        if (this.is_active == true) {
            ...
            for (let i in this.ui_types) {
                ...
                if (ui_type.object.onMouseMove != undefined) {
                    if (!dialog_active || ui_type.type == 'Dialog') {
                        if (document.fullscreenElement && document.fullscreenElement.id === this._canvas) {
                            ui_type.object.onMouseMove(x * window.devicePixelRatio, y * window.devicePixelRatio);
                        }
                        else {
    ...
    onFileInput() {
        ...
        if (this.is_active == true) {
            ...
            for (let i in this.ui_types) {
                let ui_type = this.ui_types[i];
                if (ui_type.object.onFileInput !== undefined) {
                    if (!dialog_active || ui_type.type == 'Dialog') {
                        ui_type.object.onFileInput(input);
                    }
                }
            }
        } else {
