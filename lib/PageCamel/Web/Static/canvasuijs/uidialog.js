class UIDialog {
    ...
    add(options) {
        ...
        //get text from texinput
        options.textinput = new UITextInput(this.canvas);
        options.list = new UIScrollList(this.canvas);
        options.title = new UIText();
        options.active = true;
        if (options.list_data == undefined) {
            options.list_data = [];
        }
        options.removeSelf = () => {
            //removes the dialog from the list
            var index = this.dialogs.indexOf(options);
            if (index > -1) {
        ...
        options.setList = (data) => {
            if (options.type == 'select') {
                options.list_data = data;
                options.list.find('menuList').setList(options.list_data);
            }
        }
        options.getList = () => {
            if (options.type === 'select') {
        ...
        if (options.type === 'textInput') {
            options.textinput.add({
                displaytext: options.text, name: 'input',
        ...
        if(options.type === 'euro'){
            options.textinput.add({
                displaytext: options.text, name: 'input',
                background: ['#ffffff'], foreground: '#000000', border: '#000000', border_width: 3,
                x: options.alpha_x + options.alpha_width / 2 - 100, y: options.alpha_y + options.alpha_height / 2 - 25, 
                width: 200, height: 50, font_size: 30, align: 'left', type: 'euro',
            });
            options.title.add({
                displaytext: options.displaytext,
                foreground: '#000000',
        ...
        options.getText = () => {
            if (options.textinput.find('input') !== undefined) {
                return options.textinput.find('input').getText();
            }
        }
        this.dialogs.push(options);
        return options;
    }
    recreateDialog(alpha_x, alpha_y, alpha_width, alpha_height) {
        if (this.dialogs.length > 0) {
            var dialog = this.dialogs[0];
            dialog.alpha_x = alpha_x;
            dialog.alpha_y = alpha_y;
            dialog.alpha_width = alpha_width;
            dialog.alpha_height = alpha_height;
            if(dialog.type === 'textInput' || dialog.type === 'euro'){
                dialog.text = dialog.textinput.find('input').getText();
            }
            console.log(dialog.list_data);
            this.dialogs = [];
            this.add(dialog);
        }
    }
