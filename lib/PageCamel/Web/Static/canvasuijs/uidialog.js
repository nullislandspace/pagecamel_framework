class UIDialog {
    ...
    add(options) {
        ...
        options.text_input = new UITextBox(this.canvas);
        ...
        if (options.type === 'textInput') {
            options.text_input.add({
                displaytext: options.text, name: 'room_name',
                background: ['#ffffff'], foreground: '#000000', border: '#000000', border_width: 3,
                x: options.alpha_x + options.alpha_width / 2 - 100, y: options.alpha_y + options.alpha_height / 2 - 25, width: 200, height: 50, font_size: 30, align: 'right'
            });
            options.title.add({
        ...
        options.getText = () => {
            return options.text_input.find('room_name').getText();
        }
    ...
    onMouseDown(x, y) {
        for (var i in this.dialogs) {
            ...
            dialog.text_input.onMouseDown(x, y);
    ...
    onMouseUp(x, y) {
        for (var i in this.dialogs) {
            ...
            dialog.text_input.onMouseUp(x, y);
        }
    }
    onMouseMove(x, y) {
        for (var i in this.dialogs) {
            ...
            dialog.text_input.onMouseMove(x, y);
        }
    }
    onKeyDown(e) {
        for (var i in this.dialogs) {
        ...
        var char = this.pressed_char.keydown(e);
        if (char !== undefined) {
            for (var i in this.dialogs) {
                var dialog = this.dialogs[i];
                if (dialog.type == "textInput") {
                    var obj = dialog.text_input.find("room_name");
                    if (char !== 'backspace') {
                        obj.setText(obj.getText() + char);
                    }
                    else {
                        obj.setText(obj.getText().slice(0, obj.getText().length - 1));
                    }
                }
            }
        }
