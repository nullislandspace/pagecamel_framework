...
class UIButton {
    ...
    render(ctx) {
        for (var i in this.buttons) {
            ...
            if (button.active) {
                ctx.font = button.font_size + 'px ' + font_name;
                ...
                if (button.displaytext) {
                    if (!Array.isArray(button.displaytext)) { // check if it's an array
                        button.displaytext = [button.displaytext];
                    }
                    var new_displaytext = [];
                    for (var j in button.displaytext) {
                        if (button.border_width > 0) {
                            var new_lines = autoLineBreak(ctx, button.displaytext[j], button.width - button.border_width * 2);
                        } else {
                            var new_lines = autoLineBreak(ctx, button.displaytext[j], button.width);
                        }
                        if (new_lines.length > 0) {
                            new_displaytext = [...new_displaytext, ...new_lines];
                        }
                    }
                    var yoffs = button.y + (button.height / 2) + button.font_size / 3.3;
                    if (new_displaytext.length > 1) {
                        yoffs = button.y + button.height / 2 - ((new_displaytext.length - 1) * (button.font_size * 0.80)) / 2;
                    }
                    for (j = 0; j < new_displaytext.length; j++) {
                        var text_width = ctx.measureText(new_displaytext[j]).width;
                        if (button.align == 'right') {
                            //align text right
                            ctx.fillText(new_displaytext[j], button.x + button.width - text_width - 8, yoffs);
                        }
                        else if (button.align == 'left') {
                            //align text left
                            ctx.fillText(new_displaytext[j], button.x + 8, yoffs);
                        }
                        else {
                            //align text center
                            ctx.fillText(new_displaytext[j], button.x + (button.width / 2) - (text_width / 2), yoffs);
                        }
