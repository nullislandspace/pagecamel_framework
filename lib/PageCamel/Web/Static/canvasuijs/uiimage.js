...
class UIImage {
    ...
    add(options) {
        ...
        if (options.image_hash === undefined) {
            if (options.image !== undefined && options.image.currentSrc !== undefined) {
                options.image_hash = sha256_digest(options.image.currentSrc);
                options.image_data = options.image.currentSrc;
                tableplanhandler.storeImage(options.image_hash, options.image_data);
            }
        }
    ...
    render(ctx) {
        for (var i in this.images) {
            ...
            if (image.active) {
                ...
                ctx.font = image.font_size +  'px ' + font_name;
                ...
                if (image.image !== undefined) {
                    ...
                    if (tableplanhandler.retrieveImage(image.image_hash) === undefined) {
                        if(image.image_hash !== undefined && image.image_data !== undefined){
                            tableplanhandler.storeImage(image.image_hash, image.image.src);
                        } else if (image.image_data !== undefined) {
                            image.image_hash = sha256_digest(image.image_data);
                            tableplanhandler.storeImage(image.image_hash, image.image_data);
                        }
                    }
                } else if (image.image_hash !== undefined) {
                    if (tableplanhandler.retrieveImage(image.image_hash) !== undefined) { //check if tableplanhandler already has the image
                        image.image = new Image();
                        image.image.src = tableplanhandler.retrieveImage(image.image_hash);
                        ctx.drawImage(image.image, image.x, image.y, image.width, image.height);
                        image.image_data = tableplanhandler.retrieveImage(image.image_hash);
                    }
                }
