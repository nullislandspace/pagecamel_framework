class UIListItem {
    ...
    render(ctx) {
        for (var i in this.listitems) {
            ...
            console.log(listitem.id, selected)
            if (selected == listitem.id) {
                ctx.fillRect(listitem.x + listitem.border_width / 1.8, listitem.y + listitem.font_size / 9, listitem.width - listitem.border_width * 1.2, listitem.height);
            }
            ...
            for (var j in listitem.lineitem) {
                ...
                if (type == "text") {
                    if (lineitem.align == 'right') {
                        ...
                        ctx.fillText(lineitem.displaytext, x, listitem.y + listitem.font_size * 0.8);
                    }
    ...
    onClick(x, y) {
        for (var i in this.listitems) {
            var listitem = this.listitems[i];
            var startx = listitem.x;
            var starty = listitem.y;
            var endx = listitem.width + startx;
