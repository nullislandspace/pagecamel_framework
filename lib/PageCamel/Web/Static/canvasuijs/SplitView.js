class SplitView extends UIView {
    ...
    moveLeft = (id) => {
        ...
        if (this.invoiceLeft[this.invoiceLeft.length - 1] !== undefined) {
            if (this.invoiceLeft[this.invoiceLeft.length - 1].lineitem[0].displaytext == 'BAR') {
    ...
    rebookTable = (paylist) => {
        ...
        console.log(this.views.paymentview.invoices, invoices);
        console.log('#asdasd')
        if (invoices.length > 0) {
            ...
            for (var i in this.views.paymentview.invoices) {
                var changed_invoice = this.views.paymentview.invoices[i];
                if (changed_invoice.table == this.views.paymentview.selectedTable) {
                    articles = [...changed_invoice.articles];
                    articles_index = i;
                    break;
                }
            }
            ...
            for (var i in articles) {
                for (var j in invoices) {
                    if (invoices[j].itemID == articles[i].itemID) { //delete articles when reboooking !!!!
                        articles[i].qty = invoices[j].qty;
                        rebook_articels.push({ ...articles[i] });
                    }
                }
            }
            ...
            if (paylist == 'splitLeft') {
                ...
                console.log('NEW INVOICE', new_invoice_articles);
            }
            ...
            console.log('Updated Table:', this.views.paymentview.invoices[articles_index]);
    ...
    createInvoice = (paylist) => {
        ...
        if (invoices[invoices.length - 1] !== undefined) {
            if (invoices[invoices.length - 1].lineitem[0].displaytext != 'BAR') {
                ...
                for (var i in articles) {
                    for (var j in invoices) {
                        if (invoices[j].itemID == articles[i].itemID) {
                            articles[i].qty -= invoices[j].qty;
                        }
                    }
                }
                articles = articles.filter(this.checkQTY);
                console.log(articles);
                if (articles.length == 0) {
                    this.views.paymentview.invoices.splice(articles_index, 1);
                } else {
                    this.views.paymentview.invoices[articles_index].articles = articles;
                }
                this.views.paymentview.setInvoiceSQLDB();
                console.log('Invoice Finished');
                console.log(this.views.paymentview.invoice_changes_list);
                var price_sum = 0;
                if (invoices.length > 0) {
                    try {
                        for (var article of invoices) {
