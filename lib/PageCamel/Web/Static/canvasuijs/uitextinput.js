class UITextInput {
    ...
    add(options) {
        ...
        if (options.always_active === undefined) {
            options.always_active = false;
        }
        ...
        options.setText = (text) => {
            ...
            options.textbox.textboxes[0].displaytext = text;
            return;
        }
    ...
    render(ctx) {
        for (var i in this.textinputs) {
            ...
            ctx.font = textinput.font_size +  'px ' + font_name;
    ...
    onMouseDown(x, y) {
        for (var i in this.textinputs) {
            ...
            if (x >= startx && x <= endx && y >= starty && y <= endy) {
                ...
                if (textinput.displaytext.length > 0) {
                    for (var j = 0; j < textinput.displaytext.length; j++) {
                        this.ctx.font = textinput.font_size +  'px ' + font_name;
                        ...
                        else if (textinput.align == 'right') {
                            var offset = textinput.width - this.ctx.measureText(textinput.displaytext).width - 8;
                            if (relative_mouse_down > offset + text_width - letter_width / 2 && offset + relative_mouse_down < text_width_next - letter_width_next / 2) {
                                //when cursor should be inside the text
                                textinput.cursorPos = new_text.length;
                                break
                            }
                            else if (relative_mouse_down > offset + this.ctx.measureText(textinput.displaytext).width - this.ctx.measureText(textinput.displaytext.substring(textinput.displaytext.length - 1)).width / 2) {
                                //when cursor should be at the end
                                textinput.cursorPos = textinput.displaytext.length;
                                break
                            }
                            else if (relative_mouse_down < offset + this.ctx.measureText(textinput.displaytext.substring(0, 1)).width / 2) {
                                //when cursor should be at the beginning
                                textinput.cursorPos = 0;
                                break
                            }
            ...
            else if (textinput.mouseDown && !textinput.always_active) {
    ...
    onMouseMove(x, y) {
        ...
        //$(this.canvas).css('cursor', 'default');f
    }
    onKeyDown(e) {
            ...
            if (textinput.mouseDown) {
                ...
                if (char !== 'delete' && char !== 'backspace' && char !== undefined) {
                    ...
                    if (textinput.type == 'text') {
                        ...
                        if (char == 0 && textinput.displaytext.length == 0) {
                            return;
                        }
                        textinput.displaytext = first_text_part + char + last_text_part;
                        textinput.cursorPos += 1;
                    ...
                    else if (textinput.type == 'float' && (char >= 0 || char == ',')) {
                        textinput.displaytext = first_text_part + char + last_text_part;
                        textinput.cursorPos += 1; textinput.displaytext = first_text_part + char + last_text_part;
                        textinput.always_show_cursor = true;
                    }
                    else if (textinput.type == 'euro' && (char >= 0 || char == ',')) {
                        if (char == ',' && textinput.displaytext.indexOf(',') != -1) {
                            return;
                        }
                        if (textinput.displaytext.indexOf(',') != -1 && textinput.displaytext.substring(textinput.displaytext.indexOf(',') + 1).length >= 2
                            && textinput.cursorPos > textinput.displaytext.indexOf(',')) {
                            return;
                        }
                        textinput.displaytext = first_text_part + char + last_text_part;
                        textinput.cursorPos += 1;
...
canvasuijs.addType('TextInput', UITextInput);