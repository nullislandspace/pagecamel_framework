class UIPayList {
    ...
    add(options) {
        ...
        options.setScrollPosition = (position) => {
            if (options.max_paylist_items <= options.list.length) {
                ...
                else if (position + 1 > options.list.length - options.max_paylist_items) {
                    options.scrollposition = options.list.length - options.max_paylist_items;
                    this.update();
                }
                else if (position < 0) {
    ...
    render(ctx) {
        ...
        for (var i in this.paylists) {
            ...
            paylist.listitem.render(ctx);
            paylist.arrowbutton.render(ctx);
        }
    ...
    onClick(x, y) {
        for (var i in this.paylists) {
            var paylist = this.paylists[i];
            paylist.listitem.onClick(x, y);
            paylist.arrowbutton.onClick(x, y);
        }
    ...
    onMouseDown(x, y) {
        for (var i in this.paylists) {
            var paylist = this.paylists[i];
            paylist.arrowbutton.onMouseDown(x, y);
            paylist.listitem.onMouseDown(x, y);
    ...
    onMouseUp(x, y) {
        for (var i in this.paylists) {
            var paylist = this.paylists[i];
            paylist.arrowbutton.onMouseUp(x, y);
            paylist.listitem.onMouseUp(x, y);
    ...
    onMouseMove(x, y) {
        for (var i in this.paylists) {
            var paylist = this.paylists[i];
            paylist.arrowbutton.onMouseMove(x, y);
            paylist.listitem.onMouseMove(x, y);
