class UIButtonRow {
    ...
    add(options) {
        ...
        options.recreateDialog = () => {
            if (options.change_name.find('room_name') != undefined) {
                var text = options.change_name.find('room_name').getText();
                var type = options.change_name.find('room_name').type;
                ...
                if (type == 'textInput') {
                    options.change_name.add({
                        ...
                        hover_border: '#32d600', border_width: 3, width: 700, height: 400, callback: options.dialogClose,
                        alpha_x: options.dialog_data.x, alpha_y: options.dialog_data.y, alpha_width: options.dialog_data.width, alpha_height: options.dialog_data.height,
                        type: 'textInput', displaytext: _trquote('Room Name'), text: text
                    });
                } else {
                    options.change_name.add({
                        background: ['#cecece'], foreground: '#a9a9a9', border: '#39f500', name: 'room_name',
                        ...
                        alpha_x: options.dialog_data.x, alpha_y: options.dialog_data.y, alpha_width: options.dialog_data.width, alpha_height: options.dialog_data.height,
                        type: 'conformation', displaytext: _trquote('Delete Room?')
                    });
                }
...
canvasuijs.addType('ButtonRow', UIButtonRow);