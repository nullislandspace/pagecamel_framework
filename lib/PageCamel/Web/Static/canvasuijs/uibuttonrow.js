class UIButtonRow {
    ...
    add(options) {
        ...
        options.addButton = () => {
            ...
            var x = options.x + (options.edit_button.buttons[0].width + options.gap) * options.edit_button.buttons.length;
            var height = options.edit_button.buttons[0].height;
            var y = options.edit_button.buttons[0].y;
            var width = height;
                ...
                if (buttonrow.editable) {
                    for (var j in buttonrow.edit_button.buttons) {
                        ...
                        if (x >= startx && x <= endx && y >= starty && y <= endy) {
                            buttonrow.roomSelectCallback(j);
                            buttonrow.index = j;
