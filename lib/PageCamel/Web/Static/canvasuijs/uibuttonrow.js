    ...
    add(options) {
        ...
        options.dialogClose = (action) => {
            if (action == 'ok') {
                ...
                var element_options = Object.assign({}, { displaytext: room_name }, options.elementOptions);
                ...
                element_options.callback = options.roomSelectCallback;
        ...
        options.getList = () => {
            ...
            if (!options.editable) {
                for (var i in options.button.buttons) {
                    names.push({ displaytext: options.button.buttons[i].displaytext, i: options.button.buttons[i].i });
                }
            }
            else {
                for (var i in options.edit_button.buttons) {
                    names.push({ displaytext: options.edit_button.buttons[i].displaytext, i: options.edit_button.buttons[i].i });
                }
            }
            return names;
        }
                ...
                if (buttonrow.editable) {
                    for (var j in buttonrow.edit_button.buttons) {
                        ...
                        if (x >= startx && x <= endx && y >= starty && y <= endy) {
                            console.log('room Selected ' + buttonrow.edit_button.buttons[j].i)
                            buttonrow.roomSelectCallback(buttonrow.edit_button.buttons[j].i); //select room 
                            buttonrow.index = j;
