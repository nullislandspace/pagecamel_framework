class UINumpad {
    ...
    add(options) {
        ...
        var keyvalues = [
            [{ text: _trquote('+/-') }, { text: _trquote('ZWS') }, { text: _trquote('×'), keyCode: [106] }],
            ...
            [{ text: _trquote(','), keyCode: [108, 188] }, { text: _trquote('0'), keyCode: [96, 48] }, { text: _trquote('⌫'), keyCode: [8] }]];
        ...
        if (!options.show_keys.pm) {
            keyvalues[0][0] = null;
        }
        var button_height = options.height / keyvalues.length - options.gap; //change
        if (!options.show_keys.x && !options.show_keys.ZWS && !options.show_keys.pm) {
            button_height = options.height / (keyvalues.length - 1) - options.gap;
        }
        ...
        for (var buttons_y = 0; buttons_y < keyvalues.length; buttons_y++) {
            ...
            if (!options.show_keys.x && !options.show_keys.ZWS && !options.show_keys.pm) {
                position_y = options.y + (button_height + options.gap) * (buttons_y - 1);
            }
...
canvasuijs.addType('Numpad', UINumpad);