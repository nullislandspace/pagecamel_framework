class CXScrollBar extends CXBox {
    constructor(ctx, x, y, width, height, is_relative = true, redraw = true) {
        ...
        this.pixels_per_row = 1 / this._rows;
    ...
    _getScrollbarYPos() {
        console.log('pixels per row',this.pixels_per_row)
        ...
        console.log('scroll position',this.scroll_position)
        console.log(this._ypos);
        return this.pixels_per_row * this.scroll_position; // calculate the y position of the scrollbar
    }
    _getScrollbarHeight() {
    ...
    handleEvent(event) {
        ...
        if (redraw && this._redraw) {
            console.log(this.scroll_position + " " + this._rows + " " + this._rows_per_page);
