class CXTextBox extends CXBox {
    ...
    _drawText(px=0, py=0, pwidth=this.ctx.canvas.width, pheight=this.ctx.canvas.height) {
        this.ctx.fillStyle = this.text_color;
        this.ctx.font = this.font_size + "em " + this.text_font;
        ...
        if (this.text) {
            ...
            if (this.auto_line_break) {
                for (var j in text_array) {
                    ...
                    if (this.border_width > 0) {
                        new_lines = autoLineBreak(this.ctx, text_array[j], this._width - this.border_width * 2);
                    } else {
                        new_lines = autoLineBreak(this.ctx, text_array[j], this._width);
                    }
            ...
            var yoffs = 0;
            var start_y = this._ypos + this._height / 2 - (text_lines_height - 1.8 * line_height) / 2; // get the starting y position
            for (j = 0; j < new_displaytext.length; j++) {
                ...
                if (this.text_alignment == "center") {
                    text_x = this._xpos + this._width / 2 - text_metrics.width / 2;
                }
                else if (this.text_alignment == "left") {
                    text_x = this._xpos;
                }
                else if (this.text_alignment == "right") {
                    text_x = this._xpos + this._width - text_metrics.width;
                }
            ...
            //this.ctx.strokeRect(this._xpos + 10, start_y, this._width - 20, yoffs);
        }
    }
    _drawTextBox() {
        this._drawBox();
