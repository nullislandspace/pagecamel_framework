class CXDragAndDrop extends CXButton {
    ...
    //draw the frame:
    _draw() {
        var center_x = this._xpixel + this._widthpixel / 2;
        var center_y = this._ypixel + this._heightpixel / 2;
        ctx.save(); //saves the state of canvas
        ctx.translate(center_x, center_y); //translates the canvas to the center of the dragndrop
    ...
    _rotatedragndrop = (x, y) => {
        var center_x = this._xpixel + this._widthpixel / 2;
        var center_y = this._ypixel + this._heightpixel / 2;
        var dx = x - center_x; //mouse distance from center x
        var dy = y - center_y; //mouse distance from center y
    ...
    _resize = (x, y) => {
        ...
        if (this._resize_mode == 'n-resize' || this._resize_mode == 'ne-resize' || this._resize_mode == 'nw-resize') {
            new_y = y;
            new_height = this._heightpixel + (this._ypixel - y);
            if (new_height <= this._box_size + 2) {
        ...
        if (this._resize_mode == 'w-resize' || this._resize_mode == 'nw-resize' || this._resize_mode == 'sw-resize') {
            new_x = x;
            new_width = this._widthpixel + (this._xpixel - x);
            if (new_width <= this._box_size + 2) {
    ...
    handleEvent(event) {
        ...
        if (this._dragable) {
            if (event.type == "mousedown" && this._isInside(x, y)) {
                ...
                if (this._checkResizeMode(x, y) == 'none') {
