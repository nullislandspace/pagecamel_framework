class CXDragAndDrop extends CXButton {
    ...
    _resize = (x, y) => {
        ...
        var [rx, ry] = this._rotatePoint(this._center_x, this._center_y, x, y, this._angle);
        ...
        if (this._resize_mode == 'n-resize' || this._resize_mode == 'ne-resize' || this._resize_mode == 'nw-resize') {
            new_y = ry;
            new_height = this._heightpixel + (this._ypixel - ry);
            if (new_height <= this._box_size + 2) {
        ...
        if (this._resize_mode == 'w-resize' || this._resize_mode == 'nw-resize' || this._resize_mode == 'sw-resize') {
            new_x = rx;
            new_width = this._widthpixel + (this._xpixel - rx);
            if (new_width <= this._box_size + 2) {
    ...
    _mouseMoveHandler = (x, y) => {
        ...
        var [rx, ry] = this._rotatePoint(this._center_x, this._center_y, x, y, this._angle);
        console.log('rx', rx, 'ry', ry, 'x', x, 'y', y, 'center_x', this._center_x, 'center_y', this._center_y);
        if (!this._mouse_down) {
            this._checkResizeMode(rx, ry);
        }
    ...
    _checkResizeMode(x, y) {
        if (this._show_resize_frame) {
            ...
            else if (x > this._xpixel + this._widthpixel / 2 - this._box_size_half && x < this._xpixel + this._widthpixel / 2 + this._box_size_half && y > this._ypixel - this._box_size_half * 5 && y < this._ypixel - this._box_size_half * 3) {
    ...
    handleEvent(event) {
        ...
        if (this._dragable) {
            if (event.type == "mousedown" && this._isInside(x, y)) {
                ...
                if (this._checkResizeMode(rx, ry) == 'none') {
