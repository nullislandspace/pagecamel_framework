class CXDragAndDrop extends CXButton {
    ...
    _calculateCornerPoints(x = this._xpos, y = this._ypos) {
        ...
        var [x1, y1] = this._rotatePoint(this._center_x, this._center_y, x * this._pwidth, y * this._pheight, this._angle);
        var [x2, y2] = this._rotatePoint(this._center_x, this._center_y, x * this._pwidth + this._width * this._pwidth, y * this._pheight, this._angle);
        var [x3, y3] = this._rotatePoint(this._center_x, this._center_y, x * this._pwidth + this._width * this._pwidth, y * this._pheight + this._height * this._pheight, this._angle);
        var [x4, y4] = this._rotatePoint(this._center_x, this._center_y, x * this._pwidth, y * this._pheight + this._height * this._pheight, this._angle);
    ...
    _drawResizeFrame() {
        if (this._show_resize_frame && this._resizeable) {
            ...
            //debug circle at the blue point with radius 2:
            this._ctx.strokeStyle = "blue";
            this._ctx.fillStyle = "blue";
            this._ctx.beginPath();
            this._ctx.arc(this._blue_x, this._blue_y, 2, 0, 2 * Math.PI);
            this._ctx.stroke();
            this._ctx.fill();
            this._ctx.closePath();
        }
    ...
    _mouseUpHandler = () => {
        ...
        this._blue_x = this._center_x;
        this._blue_y = this._center_y;

        //console.log('previos center', this._center_x, this._center_y);
        [this._center_x, this._center_y] = this._rotatePoint(this._center_x, this._center_y, this._xpixel + this._widthpixel / 2, this._ypixel + this._heightpixel / 2, -this._angle);
    ...
    _resize = (x, y) => {
        ...
        console.log('x:' + new_x + ' y:' + new_y + ' width:' + new_width + ' height:' + new_height);
        if (this._resize_mode == 's-resize' || this._resize_mode == 'se-resize' || this._resize_mode == 'sw-resize') {
            new_height = rotated_y - this._ypos * this._pheight;
        ...
        if (this._resize_mode == 'e-resize' || this._resize_mode == 'ne-resize' || this._resize_mode == 'se-resize') {
            new_width = rotated_x - this._xpos * this._pwidth;
        ...
        if (this._resize_mode == 'n-resize' || this._resize_mode == 'ne-resize' || this._resize_mode == 'nw-resize') {
            new_y = rotated_y;
            new_height = this._height * this._pheight + (this._ypos * this._pheight) - rotated_y;
            if (new_height <= this._box_size + 2) {
        ...
        if (this._resize_mode == 'w-resize' || this._resize_mode == 'nw-resize' || this._resize_mode == 'sw-resize') {
            new_x = rotated_x;
            new_width = this._width * this._pwidth + (this._xpos * this._pwidth) - rotated_x;
            if (new_width <= this._box_size + 2) {
        ...
        this._xpos = (this._center_x - new_width / 2) / this._pwidth;
        this._ypos = (this._center_y - new_height / 2) / this._pheight;
        //this = {...save_this};
    ...
    _move = (x, y) => {
        ...
        var new_top_left_x = dx + this._mouse_down_x - this._mouse_down_corner_distance_x;
        var new_top_left_y = dy + this._mouse_down_y - this._mouse_down_corner_distance_y;

        var [x1, y1, x2, y2, x3, y3, x4, y4] = this._calculateCornerPoints(new_top_left_x / this._pwidth, new_top_left_y / this._pheight);
        var min_x = Math.min(x1, x2, x3, x4);
        var min_y = Math.min(y1, y2, y3, y4);
        var max_x = Math.max(x1, x2, x3, x4);
        var max_y = Math.max(y1, y2, y3, y4);
        
        this._center_x = (min_x + max_x) / 2;
        this._center_y = (min_y + max_y) / 2;

        if (min_x < this._px) {
            // prevent dragndrop from going out of bounds on the left
            new_top_left_x = this._px * this._pwidth - this._mouse_down_corner_distance_x;

        }
        if (max_x > this._px + this._pwidth) {
            // prevent dragndrop from going out of bounds on the right
            new_top_left_x = (this._px + this._pwidth) * this._pwidth - this._mouse_down_corner_distance_x - this._width * this._pwidth;
        }
        if (min_y < this._py) {
            // prevent dragndrop from going out of bounds on the top
            new_top_left_y = this._py * this._pheight - this._mouse_down_corner_distance_y;
        }
        if (max_y > this._py + this._pheight) {
            // prevent dragndrop from going out of bounds on the bottom
            new_top_left_y = (this._py + this._pheight) * this._pheight - this._mouse_down_corner_distance_y - this._height * this._pheight;
        }

        this._xpos = new_top_left_x / this._pwidth;
        this._ypos = new_top_left_y / this._pheight;
        this._center_x = this._xpos * this._pwidth + this._pwidth * this._width / 2;
        this._center_y = this._ypos * this._pheight + this._pheight * this._height / 2;
    }
    //mouse move handler:
    ...
    _checkResizeMode(x, y) {
        console.log('checkResizeMode', 'x', x, 'y', y);
    ...
    checkEvent(event) {
        ...
        //get corners:
        var [x1, y1, x2, y2, x3, y3, x4, y4] = this._calculateCornerPoints();
        this._xpixel = Math.min(x1, x2, x3, x4) - this._box_size_half * 5;
        this._ypixel = Math.min(y1, y2, y3, y4) - this._box_size_half * 5;
        this._widthpixel = Math.max(x1, x2, x3, x4) - this._xpixel + this._box_size_half * 10;
        this._heightpixel = Math.max(y1, y2, y3, y4) - this._ypixel + this._box_size_half * 10;

        var result = this._checkEvent(event);

        console.log(x, this._xpixel);
        //restore locations:
        this._xpixel = x;
        this._ypixel = y;
    ...
    handleEvent(event) {
        ...
        var [rotated_x, rotated_y] = this._rotatePoint(this._center_x, this._center_y, x, y, this._angle);
        [this._relative_x, this._relative_y] = [rotated_x / this._pwidth, rotated_y / this._pheight];
