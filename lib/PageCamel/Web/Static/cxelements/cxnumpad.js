    ...
    constructor(ctx, x, y, width, height, name = "", is_relative = true, redraw = true) {
        ...
        for (var i = 0; i < this._buttons_text_block.length; i++) {
            ...
            for (var j = 0; j < this._buttons_text_block[i].length; j++) {
                var button = new CXButton(this._ctx, this._gap + j * (button_width + this._gap), this._gap + i * (button_height + this._gap), button_width, button_height, 'numpadButton', true, true);
