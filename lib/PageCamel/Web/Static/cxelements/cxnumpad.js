class CXNumPad extends CXDefault {
    constructor(ctx, x, y, width, height, is_relative = true, redraw = true) {
        super(ctx, x, y, width, height, is_relative, redraw);
        this._buttons_text = [['7', '8', '9'], ['4', '5', '6'], ['1', '2', '3'], ['+/-', '0', ',']];
        this._buttons = [];
        this._gap = 0.02;
        var button_width = (1 - this._gap * 4) / 3;
        var button_height = (1 - this._gap * 5) / this._buttons_text.length;
        this._font_size = 0.5;
        for (var i = 0; i < this._buttons_text.length; i++) {
            var row = [];
            for (var j = 0; j < this._buttons_text[i].length; j++) {
                ...
                button.text = this._buttons_text[i][j];
                button.allow_hover = true;
                button.hover_frame_color = '#ffffff';
                button.gradient = ['#f9a004', '#ff0202'];
                button.text_color = '#000000';
                button.text_size = 2;
                button.frame_color = '#ff0000';
