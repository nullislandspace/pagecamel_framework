  ...
  $.fn.extend({
    complexify: function(options, callback) {
        ...
        if (options.banMode === 'strict') {
          for (var i = 0; i < options.bannedPasswords.length; i++) {
            if (str.toLowerCase().indexOf(options.bannedPasswords[i].toLowerCase()) !== -1) {
                console.log("BANNED WORD DETECTED: " + options.bannedPasswords[i]);
                return true;
            }
          }
          return false;
        } else {
          if($.inArray(str, options.bannedPasswords) > -1) {
            console.log("Password in Banlist!");
            return true;
          }
          return false;
        }
      ...
      function evaluateSecurity() {
        ...
        console.log("Raw complexity factor: " + complexity);
