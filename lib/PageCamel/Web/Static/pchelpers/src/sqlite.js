                    ...
                    this._loadFromIndexedDB().then(async (data) => {
                        if (data) {
                            ...
                            if (data.startsWith("GZIP:")) {
                                // New gzip format - decompress with pako
                                var compressedStr = data.substring(5); // Remove "GZIP:" prefix
                                var compressedArr = this._SQLtoBinArray(compressedStr);
                                var decompressedArr = pako.inflate(compressedArr);
                                console.log("PCSqlite: Loaded gzip data from IndexedDB (" + compressedArr.length + " -> " + decompressedArr.length + " bytes)");
                                this._db = new SQL.Database(decompressedArr);
                            } else {
                                // Try LZString decompress (for backward compatibility with old data)
                                var decompressed = LZString.decompress(data);
                                if (decompressed && decompressed.length > 0) {
                                    console.log("PCSqlite: Loaded LZString data from IndexedDB (" + data.length + " -> " + decompressed.length + " bytes)");
                                    this._db = new SQL.Database(this._SQLtoBinArray(decompressed));
                                } else {
                                    // Assume uncompressed
                                    console.log("PCSqlite: Loaded uncompressed data from IndexedDB (" + data.length + " bytes)");
                                    this._db = new SQL.Database(this._SQLtoBinArray(data));
                                }
                            }
                        }
                        else {
