...
export class PCWebsocket {
    ...
    constructor(caching = false, debug = false) {
        ...
        this._deltatime = 250;
        //Cache send delta time in ms (retry interval if not acknowledged)
        this._cache_delta_time = 1000;
    ...
    /**
     * Force send the next cached message immediately, bypassing the retry delay.
     * Use this when the user is navigating away and we need to sync urgently.
     * @returns TRUE if a message was sent
     */
    flushOutbox() {
        try {
            if (this._db && this._dbcaching && this._isconnected) {
