...
class UILine {
    ...
    add(options) {
        ...
        var keyvalues = [['+/-', 'ZWS', '⌫'], ['7', '8', '9'], ['4', '5', '6'], ['1', '2', '3'], ['x', '0', ',']];
    ...
    add(options) {
        console.log(options.displaytext)
    ...
    add(options) {
        ...
        options.setSQLData = () => {
            executeSQL(`CREATE TABLE IF NOT EXISTS tableplan(id INTEGER PRIMARY KEY AUTOINCREMENT,\
                name TEXT,\
                data TEXT\
                );`);//id, name, data
            executeSQL(`DELETE FROM tableplan WHERE name='${options.name}';`);

            var data = options.undoable[options.undoable.length - 1]; //get current dragndrop data
            data = JSON.stringify(data);
            executeSQL(`INSERT INTO tableplan (name, data)\
            VALUES ('${options.name}','${data}');`);
        }
        options.getSQLData = () => {
            var data = executeSQL(`SELECT data FROM tableplan WHERE name='${options.name}'`);
            var data = JSON.parse(data[0].values[0]);
            if (data.length > 0) {
                for(var i in data){
                    data[i].callback = options.addToUndoable;
                    data[i].isSelected = options.isSelected;
                    this.dragndrop.add(data[i]);
                }
            ...
            if (uitableplan.editable == true && (uitableplan.circle == true || uitableplan.rect == true) && uitableplan.draw.mousedown_x != null) {
                ...
                if (uitableplan.rect) {
                    this.dragndrop.add({
                        ...
                        x: x, y: y, width: width, height: height, font_size: 25, angle: 0, callback: uitableplan.addToUndoable, isSelected: uitableplan.isSelected
                    });
                }
                else if (uitableplan.circle) {
                    this.dragndrop.add({
    ...
    render(ctx) {
        for (var i in this.circles) {
            ...
            ctx.font = circle.font_size + 'px Courier';
