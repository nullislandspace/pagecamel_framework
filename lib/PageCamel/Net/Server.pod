...
PageCamel::Net::Server - Extensible, general Perl server engine

=head1 This is a fork, not the original!

This is a fork of the L<Net::Server> module to better support the PageCamel framework. It
has a lot of patches and bugfixes applied, and should work as a plug-in replacement for
L<Net::Server> in most cases (just change the module names in your code).

There are some key areas where PageCamel::Net::Server is B<not> backwards compatible, like
exception handling during process_request(), the addition of a debugging shim and such.

For a detailed view of what has changed, compared to the original L<Net::Server>, you should
take a look at the official mercurial repository: L<https://cavac.at/public/mercurial/PageCamel-Net-Server/>
...
Not all hooks might be called on every connection. This depends
on configuration and circumstances. For example, if you respond
negative in the allow_deny_hook(), naturally your process_request()
method will not be called. The same goes for certain error conditions,
like the client failing SSL negotiation.
...
=item C<$self-E<gt>processing_error_hook(@errors)>

Most of the processing hooks are run within an eval to catch unhandled
exceptions. If there is such an exception, processing_error_hook will be
called with $@ as argument.

The default is to close the child (or in case of single-process/single-thread)
to stop the server.

You may, or may not want to override this to do your own error handling and/or
logging.

You MUST return a true value if you do NOT want the child (or server) to shut
down.

B<WARNING:> If you encounter another unhandled exception during
C<processing_error_hook()> and you are running in a multi-process
server, this I<may or may not> have I<undefined consequences>. This
depends on the server configuration, connection status, operating system,
weather, alignment of stars and possibly roll of a dice (see also L<Acme::XKCD::DebianRandom>).

For example, the server can keep accepting connections but never
calling your handling functions, leading to stalled connections.
So make sure you handle B<all> exceptions during  C<processing_error_hook()>
and also make sure you do B<not> exit() or close the running process
in any other way!
...
Original author: Paul Seamons <paul at seamons.com>

This fork: Rene Schickbauer <cavac at cpan.org>
