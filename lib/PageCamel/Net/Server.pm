...
sub run_client_connection {
    ...
    eval { # Make sure handling the client does not crash the server and we can clean up as best as possible
        ...
        if ($ok) {
            $self->process_request($c);   # This is where the core functionality of a PageCamel::Net::Server should be.
        } else {
...
sub initialize_logging {
    ...
    if ($prop->{'log_file'} =~ /^([a-zA-Z]\w*(?:::[a-zA-Z]\w*)*)$/) {
        my $pkg  = "PageCamel::Net::Server::Log::$prop->{'log_file'}";
        (my $file = "$pkg.pm") =~ s|::|/|g;
...
sub pre_bind { # make sure we have good port parameters
    ...
    if ($self->isa('PageCamel::Net::Server::MultiType')) {
        my $base = delete($prop->{'_recursive_multitype'}) || PageCamel::Net::Server::MultiType->net_server_type;
...
sub port_info {
    ...
    return PageCamel::Net::Server::Proto->parse_info($port, $host, $proto, $ipv, $self);
}

sub proto_object {
    my ($self, $info) = @_;
    return PageCamel::Net::Server::Proto->object($info, $self);
}
...
sub post_bind { # secure the process and background it
    ...
    if (! defined $prop->{'group'}) {
        ...
        $prop->{'group'} = eval { get_gid($prop->{group}) };
        $self->fatal(my $e = $@) if $@;
    }
    ...
    $SIG{'CLD'} = \&sig_chld; # catch children (mainly for Fork and PreFork but works for any chld)
...
sub post_accept {
    ...
    if (! $prop->{'no_client_stdout'}) {
        ...
        if ($prop->{'tie_client_stdout'} || ($client->can('tie_stdout') && $client->tie_stdout)) {
            ...
            tie *STDOUT, 'PageCamel::Net::Server::TiedHandle', $client, $prop->{'tied_stdout_callback'} or die "Couldn't tie STDOUT: $!";
            tie *STDIN,  'PageCamel::Net::Server::TiedHandle', $client, $prop->{'tied_stdin_callback'}  or die "Couldn't tie STDIN: $!";
...
sub register_sig_pass {
    ...
    require PageCamel::Net::Server::SIG;
    foreach my $sig (map {split /\s*,\s*/, $_} @$ref) {
        my $code = PageCamel::Net::Server::SIG::sig_is_registered($sig);
        if ($code) {
            $self->log(2, "Installing passthrough for $sig even though it is already registered.");
        } else {
            $code = ref($SIG{$sig}) eq 'CODE' ? $SIG{$sig} : undef;
        }
        PageCamel::Net::Server::SIG::register_sig($sig => sub { $self->sig_pass($sig); $code->($sig) if $code; });
...
package PageCamel::Net::Server::TiedHandle;
