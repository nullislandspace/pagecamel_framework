# -*- perl -*-
#
#  PageCamel::Net::Server::Proto - PageCamel::Net::Server Protocol compatibility layer
#
#  $Id$
#
#  Original author:
...
sub parse_info {
    ...
    if (ref($port) eq 'HASH') {
        ...
        $proto = $1 if $port =~ s{ (?<=[\w*\]]) [,|\s:/]+ (tcp|udp|ssl|ssleay|unix|unixdgram|\w+(?: ::\w+)+) $ }{}xi # allow for 80/tcp or 200/udb or 90/PageCamel::Net::Server::Proto::TCP
...
sub object {
    ...
    if ($proto_class !~ /::/) {
        ...
        $proto_class = "PageCamel::Net::Server::Proto::" .uc($proto_class);
    }
...
=head1 SYNOPSIS
    ...
    NOTE: beginning in PageCamel::Net::Server 2.005, the default value for
    ...
    # PageCamel::Net::Server::Proto and its accompanying modules are not
    # intended to be used outside the scope of PageCamel::Net::Server.
    ...
    my @info = PageCamel::Net::Server::Proto->parse_info(
        ...
        $server_obj,      # PageCamel::Net::Server object
    );

    my @raw_info = PageCamel::Net::Server::Proto->get_addr_info($host, $port, $proto);
    # returns arrayref of resolved ips, ports, and ipv values

    my $sock = PageCamel::Net::Server::Proto->object({
        port  => $port,
        host  => $host,
        proto => $proto,
    ...
    # PageCamel::Net::Server::Proto will attempt to interface with
    # sub modules named similar to PageCamel::Net::Server::Proto::TCP
    # Individual sub modules will be loaded by
    # PageCamel::Net::Server::Proto as they are needed.

    use PageCamel::Net::Server::Proto::TCP; # or UDP or UNIX etc
    ...
    my $sock = PageCamel::Net::Server::Proto::TCP->object({
        port  => $port,
        host  => $host,
        proto => $proto,
    ...
    # Use the facilities of the passed PageCamel::Net::Server object.
...
PageCamel::Net::Server::Proto is an intermediate module which returns IO::Socket
style objects blessed into its own set of classes (ie
PageCamel::Net::Server::Proto::TCP, PageCamel::Net::Server::Proto::UNIX).

Only three or four protocols come bundled with PageCamel::Net::Server.  TCP, UDP,
...
The protocol that is passed to PageCamel::Net::Server can be the name of another
...
class.  If PageCamel::Net::Server::Proto::TCP was passed, it would get that
class.  If a bareword, such as tcp, udp, unix, unixdgram or ssleay, is
passed, the word is uppercased, and post pended to
"PageCamel::Net::Server::Proto::" (ie tcp = PageCamel::Net::Server::Proto::TCP).

=head1 METHODS

Protocol names used by the PageCamel::Net::Server::Proto should be sub classes of
...
port, and a PageCamel::Net::Server style server object.

=item log_connect

Log that a connection is about to occur.  Use the facilities of the
passed PageCamel::Net::Server object.  This should be an informative string
...
PageCamel::Net::Server protocol.  Return the port that is being used by this
module.  If the underlying type is UNIX then port will actually be
the path to the unix socket file.

=item NS_host

PageCamel::Net::Server protocol.  Return the protocol that is being used by this
...
module classes and to PageCamel::Net::Server::Proto itself.  For tcp,
...
The PageCamel::Net::Server::Proto::object method returns a list of objects corresponding
...
You can see what PageCamel::Net::Server::Proto parsed out by looking at
...
examples:
    ...
    $def_proto = undef;
    $def_ipv   = undef;
    @info = PageCamel::Net::Server::Proto->parse_info($port,$def_host,$def_proto,$def_ipv);
    # @info = {
    #     host  => 'default-domain.com',
    #     port  => 20203,
    #     proto => 'tcp', # will use PageCamel::Net::Server::Proto::TCP
    #     ipv   => *, # IPv*
    # };
    ...
    #     host  => 'someother.com',
    #     port  => 20203,
    #     proto => 'udp', # will use PageCamel::Net::Server::Proto::UDP
    #     ipv   => *,
    # };

    # example 4 #----------------------------------

    $port      = "someother.com:20203/PageCamel::Net::Server::Proto::UDP";
    $def_host  = "default-domain.com";
    $def_proto = "TCP";
    $def_ipv   = 4;
    @info = PageCamel::Net::Server::Proto->parse_info($port,$def_host,$def_proto,$def_ipv);
    ...
    #     host  => 'someother.com',
    #     port  => 20203,
    #     proto => 'PageCamel::Net::Server::Proto::UDP',
    #     ipv   => 4,
    # };
    ...
    $def_host  = "default-domain.com";
    $def_proto = "tcp";
    @info = PageCamel::Net::Server::Proto->parse_info($port,$def_host,$def_proto);
    # @info = {
    #     host  => 'someother.com',
    #     port  => 20203,
    #     proto => 'MyObject::TCP',
    ...
    #     proto => 'unix', # will use PageCamel::Net::Server::Proto::UNIX
    ...
    #     host  => '*', # irrelevant for UNIX socket
    #     port  => '/tmp/mysock.file', # not really a port
    #     proto => 'unixdgram', # will use PageCamel::Net::Server::Proto::UNIXDGRAM
    #     ipv   => '*', # irrelevant for UNIX socket
    # };
    ...
    #     host  => '*', # irrelevant for UNIX socket
    #     port  => '/tmp/mysock.file', # not really a port
    #     proto => 'unix', # will use PageCamel::Net::Server::Proto::UNIXDGRAM
    ...
    #     host  => 'someother.com',
    #     port  => 20203,
    #     proto => 'ssleay', # will use PageCamel::Net::Server::Proto::SSLEAY
    #     ipv   => *,
    # };
