# -*- perl -*-
#
#  PageCamel::Net::Server::SIG - Safer signals
#
#  $Id$
#
#  Original author:
...
package PageCamel::Net::Server::SIG;
#---AUTOPRAGMASTART---
use 5.030;
use strict;
use warnings;
...
sub register_sig {
    ...
    if (! $ref) {
        ...
        $SIG{$sig} = sub{ $PageCamel::Net::Server::SIG::_SIG{$sig} = 1 };
    } else {
...
PageCamel::Net::Server::SIG - adpf - Safer signal handling
...
=head1 SYNOPSIS
    ...
    register_sig(PIPE => 'IGNORE',
                 ...
                 CLD => sub { 1 while waitpid(-1, WNOHANG) > 0; },
                 );
...
have been implemented in a more safe algorithm.  PageCamel::Net::Server::SIG
provides backwards compatibility, while still working reliably with
newer releases.

Using a property of the select() function, PageCamel::Net::Server::SIG attempts
to fix the unsafe problem.  If a process is blocking on select() any
signal will short circuit the select.  Using this concept,
PageCamel::Net::Server::SIG does the least work possible (changing one bit from 0
...
in creating PageCamel::Net::Server::SIG and sigtest.pl.
