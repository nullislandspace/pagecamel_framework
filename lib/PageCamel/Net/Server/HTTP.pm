# -*- perl -*-
#
#  PageCamel::Net::Server::HTTP - Extensible Perl HTTP base server
#
#  $Id$
#
#  Original author:
...
sub post_configure {
    ...
    my $d = {
        timeout_header  => 15,
        timeout_idle    => 60,
        server_revision => __PACKAGE__."/$PageCamel::Net::Server::VERSION",
...
sub process_headers {
    ...
    my $type = $PageCamel::Net::Server::HTTP::ISA[0];
    $type = $PageCamel::Net::Server::MultiType::ISA[0] if $type eq 'PageCamel::Net::Server::MultiType';
...
sub exec_cgi {
    ...
    my $done = 0;
    my $pid;
    PageCamel::Net::Server::SIG::register_sig(CHLD => sub {
        while (defined(my $chld = waitpid(-1, POSIX::WNOHANG()))) {
            $done = ($? >> 8) || -1 if $pid == $chld;
            last unless $chld > 0;
        }
    });
    PageCamel::Net::Server::SIG::register_sig(CLD => sub {
        while (defined(my $chld = waitpid(-1, POSIX::WNOHANG()))) {
            $done = ($? >> 8) || -1 if $pid == $chld;
            last unless $chld > 0;
        }
    ...
    while (!$done) {
        ...
        PageCamel::Net::Server::SIG::check_sigs();
        for my $fh (@$o) {
    ...
    PageCamel::Net::Server::SIG::unregister_sig('CHLD');
    PageCamel::Net::Server::SIG::unregister_sig('CLD');
}
...
PageCamel::Net::Server::HTTP - very basic PageCamel::Net::Server based HTTP server class

=head1 TEST ONE LINER

    perl -e 'use base qw(PageCamel::Net::Server::HTTP); main->run(port => 8080)'
...
Even though PageCamel::Net::Server::HTTP doesn't fall into the normal parallel of
the other PageCamel::Net::Server flavors, handling HTTP requests is an often
requested feature and is a standard and simple protocol.

PageCamel::Net::Server::HTTP begins with base type MultiType defaulting to
PageCamel::Net::Server::Fork.  It is easy to change it to any of the other
PageCamel::Net::Server flavors by passing server_type => $other_flavor in the
...
header.  This is one change from the other standard PageCamel::Net::Server base
classes.
...
PageCamel::Net::Server will handle the header negotiation for you.

Note: PageCamel::Net::Server::HTTP has no concept of document root or script
aliases or default handling of static content.  That is up to the
consumer of PageCamel::Net::Server::HTTP to work out.

PageCamel::Net::Server::HTTP comes with a basic %ENV display installed as the
default process_http_request method.

=item C<process_request>

This method has been overridden in PageCamel::Net::Server::HTTP - you should not
use it while using PageCamel::Net::Server::HTTP.  This overridden method parses
...
augment PageCamel::Net::Server::HTTP with document root and virtual host
capabilities.
...
In addition to the command line arguments of the PageCamel::Net::Server base
...
Defaults to PageCamel::Net::Server::HTTP/$PageCamel::Net::Server::VERSION.
...
module.
    ...
    PageCamel::Net::Server::HTTP->run(app => { # loses order of matching
      '/' => sub { ... },
      '/foo' => sub { ... },
      '/bar' => '/path/to/some.cgi',
    });

    PageCamel::Net::Server::HTTP->run(app => [
      '/' => sub { ... },
      '/foo' => sub { ... },
      '/bar' => '/path/to/some.cgi',
    ]);
...
L<PageCamel::Net::Server::PreFork>,
