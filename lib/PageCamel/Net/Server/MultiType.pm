# -*- perl -*-
#
#  PageCamel::Net::Server::MultiType - PageCamel::Net::Server personality
#
#  $Id$
#
#  Original author:
...
package PageCamel::Net::Server::MultiType;
#---AUTOPRAGMASTART---
use 5.030;
use strict;
use warnings;
...
sub run {
    ...
    foreach my $type (@{ $prop->{'server_type'} || []}) {
        ...
        my $pkg = ($type =~ /::/) ? $type : "PageCamel::Net::Server::$type";
        (my $file = "$pkg.pm") =~ s{::}{/}g;
        eval { require $file };
        if ($@){
        ...
        # handle items like HTTP and PSGI that aren't true PageCamel::Net::Server flavors, but themselves are MultiType
        if ($pkg->isa(__PACKAGE__)) {
            my $type = $self->default_server_type || 'Single';
            $type = ($type =~ /^(\w+)$/) ? $1 : next; # satisfy taint
            my $_pkg = ($type =~ /::/) ? $type : "PageCamel::Net::Server::$type";
        ...
        # cludgy - doesn't allow multiple PageCamel::Net::Server::MultiType servers within same process
        # but it is probably better than modifying our child's class for it
        @PageCamel::Net::Server::MultiType::ISA = ($pkg);
        last;
    }
...
=head1 SYNOPSIS
    ...
    PageCamel::Net::Server::MultiType->run(server_type => \@types);

=head1 DESCRIPTION

Please read the pod on PageCamel::Net::Server first.  This module is a
personality, or extension, or sub class, of the PageCamel::Net::Server module.

This personality is intended to allow for easy use of multiple
PageCamel::Net::Server personalities.  Given a list of server types,
PageCamel::Net::Server::MultiType will require one at a time until it finds one
...
class, PageCamel::Net::Server::MultiType contains one other configurable
parameter.
...
server_types.  At execution, PageCamel::Net::Server::MultiType will find the
...
C<PageCamel::Net::Server::MultiType> allows for the use of a configuration file
...
There are no additional hooks in PageCamel::Net::Server::MultiType.
