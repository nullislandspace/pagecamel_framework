# -*- perl -*-
#
#  PageCamel::Net::Server::Multiplex - PageCamel::Net::Server personality
#
#  $Id$
#
#  Original author:
...
sub loop {
    ...
    foreach my $sock ( @{ $prop->{sock} } ) {
        if (PageCamel::Net::Server::SOCK_DGRAM == $sock->getsockopt(Socket::SOL_SOCKET(),Socket::SO_TYPE())) {
            $mux->add($sock);
        } else {
            $mux->listen($sock);
        }
    }
    $mux->set_callback_object(PageCamel::Net::Server::Multiplex::MUX->init($self));

    ###
    ### Use PageCamel::Net::Server::SIG for safe signal handling.
    ###
...
sub setup_client_connection {
    ...
    ### Copied from PageCamel::Net::Server::post_accept...
    $prop->{requests} ++;
    ...
    ### Copied from PageCamel::Net::Server::run_client_connection...
...
# Compatibility interface for PageCamel::Net::Server
sub run_dequeue {
    confess "&$PageCamel::Net::Server::Multiplex::MUX::ISA[0]\::run_dequeue never defined";
}

sub mux_connection {}
sub mux_input {
    confess "&$PageCamel::Net::Server::Multiplex::MUX::ISA[0]\::mux_input never defined";
}
sub mux_eof {}
sub mux_close {}
sub mux_timeout {
    confess "&$PageCamel::Net::Server::Multiplex::MUX::ISA[0]\::mux_timeout never defined";
}

package PageCamel::Net::Server::Multiplex::MUX;
...
our @ISA = qw(PageCamel::Net::Server::Multiplex);
...
# The new() routine is passed the PageCamel::Net::Server object.  It
# is meant to create the client specific callback object.
# Note that the $net_server->{server} property hash may be
# modified by future connections through PageCamel::Net::Server.
...
sub mux_connection {
    ...
    if ($net_server->setup_client_connection($mux)) {
        ...
        my $client_object = PageCamel::Net::Server::Multiplex::MUX->new($net_server, $fh);
...
PageCamel::Net::Server::Multiplex - Multiplex several connections within one process

=head1 SYNOPSIS

    package MyPlexer;

    use base qw(PageCamel::Net::Server::Multiplex);
...
This takes some nice features of PageCamel::Net::Server (like the server listen
...
See examples/samplechat.pl distributed with PageCamel::Net::Server for a simple
...
The following represents the client processing program flow:
    ...
    $self->{server}->{client} = PageCamel::Net::Server::Proto::TCP->accept();  # NOTE: Multiplexed with mux_input() below
    ...
    $self->post_accept_hook; # PageCamel::Net::Server style

    if ($self->allow_deny
        && $self->allow_deny_hook) {

      # (PageCamel::Net::Server style $self->process_request() is never called.)
...
The loop() method of PageCamel::Net::Server has been overridden to run the loop
routine of IO::Multiplex instead.  The PageCamel::Net::Server methods may access
the IO::Multiplex object at C<$self-E<gt>{mux}> if desired.  The
IO::Multiplex methods may access the PageCamel::Net::Server object at
C<$self-E<gt>{net_server}> if desired.

The process_request() method is never used with this personality.

The other PageCamel::Net::Server hooks and methods should work the same.
...
convenience and to more closely emulate the PageCamel::Net::Server model.
However, unlike some other PageCamel::Net::Server personalities, you should never
...
PageCamel::Net::Server.  Any values within it that this object may need to use
...
interface, you cannot use the PageCamel::Net::Server style check_for_dequeue /
...
interface to comply with the PageCamel::Net::Server::Fork style run_dequeue but
