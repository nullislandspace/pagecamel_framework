...
use PageCamel::Web::Mercurial::ROProxy;
use PageCamel::Web::Mercurial::RWProxy;
...
sub parse_post_data {
    ...
    if($ua->{headers}->{'Content-Type'} =~ /application\/x\-www\-form\-urlencoded/) {
        my %postparams;
    ...
    foreach my $key (keys %{$ua->{postparams}}) {
        my $bla = ref $ua->{postparams}->{$key};
        if(ref $ua->{postparams}->{$key} eq '' && !is_utf8($ua->{postparams}->{$key})) {
            $ua->{postparams}->{$key} = decode_utf8($ua->{postparams}->{$key});
        }
    }
    return $ok;
}
